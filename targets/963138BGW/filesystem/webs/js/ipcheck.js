...
function checkBcastOrSubnet( lan, mask, ipaddr )
{
  var iptmpvalue   = ipaddr.split(".");
  var masktmpvalue = mask.split(".");
  var lantmp       = lan.split(".");
  var len = lantmp.length;
  var i, Lan_IP, Sub_Mask, check_IP;
  for(i=0; i<len; i++) {
        Lan_IP   = ( (Lan_IP<<8)   | parseInt(lantmp[i]) );
        Sub_Mask = ( (Sub_Mask<<8) | parseInt(masktmpvalue[i]) );
        check_IP = ( (check_IP<<8) | parseInt(iptmpvalue[i]) );
  };

  var net_IP = Lan_IP & Sub_Mask;
  var bcast_IP = net_IP+(~Sub_Mask);
  if ( (check_IP == bcast_IP))
  {
      alert("It is Broadcast address.\n");
      return true;
  }

  if ( (check_IP == net_IP))
  {
      alert("It is Subnet address.\n");
      return true;
  }
  return false;
}
