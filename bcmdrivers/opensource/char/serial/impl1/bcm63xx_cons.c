...
#endif
	.membase    = (void *)IO_ADDRESS(UART_PHYS_BASE),
...
#endif
	.membase    = (void *)IO_ADDRESS(UART1_PHYS_BASE),
...
bcm63xx_console_write(struct console *co, const char *s, unsigned int count)
{
   ...
   /*
    * Don't touch intMask, this path (printk) is not protected against
    * the printf path.
    */
    ...
    for (i = 0; i < count; i++) {
        ...
        */
        ...
        /* no need to check tx fifo level here, we know we are close to empty,
         * so sending one more byte will not hurt.
         */
        if (s[i] == '\n') {
