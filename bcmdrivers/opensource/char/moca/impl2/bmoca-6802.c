...
static int  hw_specific_init( struct moca_priv_data *priv )
{
	...
	MOCA_WR(0x10800e08, 9000); // EPORT_UMAC_RX_MAX_PKT_SIZE_OFFSET - Allow jumbo frames up to 9000 bytes
	MOCA_WR(0x10800814, 9000); // EPORT_UMAC_FRM_LEN_OFFSET
...
/*
 * The MoCA Clock APIs are used only for MoCA Chip ID = 0x680200C0
 * The clk_set_rate() API causes stack overflow error. Since MoCA chip
 * currently in use on BHR5 is 0x680300b0 we are commenting this code.
 * We expect the stack overflow to be fixed in one of the the newer 
 * SDK releases. 
 * Otherwise, we will fix the stack overflow by moving the memory allocation
 * for the following data structures outside the kernel process stack by 
