...
#if defined(CONFIG_BCM963138) || defined(CONFIG_BCM963148) || defined(CONFIG_BCM96848)
    if (BcmHalMapInterrupt((FN_HANDLER)ext_timer_isr, INTERRUPT_ID_TIMER0, INTERRUPT_ID_TIMER0))
    {
        spin_unlock_irqrestore(&timer_spinlock, flags);
        return -1;
    }
#if !defined(CONFIG_ARM)
    BcmHalInterruptEnable(INTERRUPT_ID_TIMER0);
#endif
    if (BcmHalMapInterrupt((FN_HANDLER)ext_timer_isr, INTERRUPT_ID_TIMER1, INTERRUPT_ID_TIMER1))
    {
        spin_unlock_irqrestore(&timer_spinlock, flags);
        return -1;
    }
#if !defined(CONFIG_ARM)
    BcmHalInterruptEnable(INTERRUPT_ID_TIMER1);
#endif
    if (BcmHalMapInterrupt((FN_HANDLER)ext_timer_isr, INTERRUPT_ID_TIMER2, INTERRUPT_ID_TIMER2))
    {
        spin_unlock_irqrestore(&timer_spinlock, flags);
        return -1;
    }
#if !defined(CONFIG_ARM)
    BcmHalInterruptEnable(INTERRUPT_ID_TIMER2);
#endif
    if (EXT_TIMER_NUM > 3)
    {
        if (BcmHalMapInterrupt((FN_HANDLER)ext_timer_isr, INTERRUPT_ID_TIMER3, INTERRUPT_ID_TIMER3))
        {
            spin_unlock_irqrestore(&timer_spinlock, flags);
            return -1;
        }
#if !defined(CONFIG_ARM)
        BcmHalInterruptEnable(INTERRUPT_ID_TIMER3);
#endif
    }
#endif
    initialized = 1;
