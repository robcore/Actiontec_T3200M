...
void *bdmf_alloc(size_t size)
{
     ...
     *GFP_ATOMIC.*/
    return kmalloc(size + sizeof(uint32_t), GFP_ATOMIC);
}
...
void *bdmf_alloc_uncached(int size, uint32_t *phys_addr_p)
{
...
void bdmf_free_uncached(void *virt_p, uint32_t phys_addr, int size)
{
...
void *bdmf_alloc_uncached(int size, uint32_t *phys_addr_p)
{
    return dma_alloc_coherent(NULL, size, phys_addr_p, GFP_ATOMIC);
}

void bdmf_free_uncached(void *virt_p, uint32_t phys_addr, int size)
{
    dma_free_coherent(NULL, size, virt_p, phys_addr);
}
...
void bdmf_reent_fastlock_unlock(bdmf_reent_fastlock *lock)
{
    ...
    if (!reent_fastlock->refcnt[cpu_id]) /* Last time - release the lock */
        spin_unlock(&reent_fastlock->lock);
