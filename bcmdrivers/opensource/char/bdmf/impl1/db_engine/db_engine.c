...
#define BDMF_DB_MAGIC_FREE_SET           (('f'<<24) | ('s'<<16) | ('e'<<8) | 't')
    ...
    /** Get next element */
    bdmf_db_key_t (*entry_get_next)(const bdmf_db_set_t *this, bdmf_db_key_t cur);

    /** Add new entry. returns 0 if ok */
    int (*entry_new)(bdmf_db_set_t *this, bdmf_db_key_t key, void *data);

    /** Delete entry */
    int (*entry_delete)(bdmf_db_set_t *this, bdmf_db_entry_t *entry);
    ...
    bdmf_db_key_t (*handle_to_key)(const bdmf_db_set_t *this, const bdmf_db_entry_t *entry);

    /** Convert entry key to entry handle */
    bdmf_db_entry_t *(*key_to_handle)(const bdmf_db_set_t *this, bdmf_db_key_t key);
...
/** Get next element */
static bdmf_db_key_t dbbe_array_entry_get_next(const bdmf_db_set_t *this, bdmf_db_key_t key)
{
    assert(this->entry.data);
    ...
    {
        bdmf_db_entry_t *entry = (bdmf_db_entry_t *)this->entry.data + key;
...
/** Convert entry handle to entry key */
static inline bdmf_db_key_t dbbe_array_handle_to_key(const bdmf_db_set_t *this, const bdmf_db_entry_t *entry)
{
    bdmf_db_key_t key;

    assert(this);
    assert(entry);
    assert(this->magic == BDMF_DB_MAGIC_ACTIVE_SET);
    assert(this->entry.data);

    key = entry - (bdmf_db_entry_t *)this->entry.data;
    if ((unsigned)key >= this->max_entries)
...
/** Convert entry key to entry handle */
static inline bdmf_db_entry_t *dbbe_array_key_to_handle(const bdmf_db_set_t *this, bdmf_db_key_t key)
{
    assert(this);
    assert(this->magic == BDMF_DB_MAGIC_ACTIVE_SET);
    assert(this->entry.data);
    ...
    return (bdmf_db_entry_t *)this->entry.data + key;
}
 ...
 */
static int ddbe_set_new(bdmf_db_set_t *this, bdmf_db_key_t key, void *data)
{
    bdmf_db_entry_t *entry = this->key_to_handle(this, key);
 ...
 */
static int ddbe_record_new(bdmf_db_set_t *this, bdmf_db_key_t key, void *data)
{
    bdmf_db_record_t *record = (bdmf_db_record_t *)this->key_to_handle(this, key);
...
/** Delete entry */
static int ddbe_entry_delete(bdmf_db_set_t *this, bdmf_db_entry_t *entry)
{
    if (!entry)
        return BDMF_ERR_PARM;
    if (!(entry->flags & BDMF_DB_FLAG_VALID))
