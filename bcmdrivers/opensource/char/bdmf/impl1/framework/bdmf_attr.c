...
static int _bdmf_attr_index_val(char **ppbuf, char **index_val)
{
    char *pbuf = *ppbuf;
    char ob = *pbuf;
...
static int _bdmf_attr_string_split(char *attr,
        struct bdmf_attr_name_value **p_pairs, int *p_npairs)
{
    ...
    while (pbuf && *pbuf)
    {
        ...
        if (pbuf)
        {
            ...
            *pbuf = 0;
            ...
            case '[': /* indexed attribute */
                ++pbuf;
                rc = _bdmf_attr_index_val(&pbuf, &index);
                if (rc)
                    goto out_of_loop;
                if (!pbuf || *pbuf != ']')
                {
        ...
        if (value)
        {
             ...
             */
            rc = _bdmf_attr_index_val(&pbuf, &value);
            if (rc)
                goto out_of_loop;
        ...
        struct bdmf_attr_name_value **p_pairs, int *p_npairs)
{
    ...
    return _bdmf_attr_string_split(buf, p_pairs, p_npairs);
}
 ...
 * internal helper
 */
static struct bdmf_attr *_bdmf_attr_by_name(struct bdmf_attr *aattr,
        const char *name)
{
...
static int _bdmf_delete_aggr(bdmf_object_handle mo, struct bdmf_attr *attr, bdmf_index index)
{
    ...
    rc = rc ? rc : attr->delete(mo, attr, index);
    if (!rc)
    {
...
/* Clear attribute containg referenced object */
        ...
        bdmf_index *index, const void *buffer, uint32_t size, int is_add)
{
    ...
    if (size < attr->size && attr->type != bdmf_attr_buffer && buffer)
        return BDMF_ERR_OVERFLOW;
    if (attr->type == bdmf_attr_aggregate )
        return _bdmf_set_aggr(mo, attr, index, buffer, size, is_add);
    if (attr->type == bdmf_attr_object)
        return _bdmf_attrelem_set_as_ref(mo, attr, index, *(struct bdmf_object **)buffer, is_add);
    if (is_add)
        ...
        char *set)
{
    ...
    rc = _bdmf_attr_string_split(set, &pairs, &npairs);
    if (rc)
        return rc;
...
int bdmf_configure(bdmf_object_handle mo, const char *set)
{
    ...
    bdmf_lock();
    rc = _bdmf_configure(mo, mo->drv->aattr, pbuf);
...
int bdmf_attr_get_hlp(const char *attrs, const char *name, char *val,
        uint32_t val_length)
{
    char *buf = bdmf_alloc(strlen(attrs) + 1);
    char *pbuf = buf;
    char *token, *attr_val;
...
void bdmf_attr_help(const struct bdmf_attr *attr, char *buffer, uint32_t size)
{
    ...
    for (step = 0; size && step <= 10; step++)
    {
        ...
        case 5:
            ...
            break;
        case 6:
            step_len = snprintf(buffer, size, "%c", attr->delete ? 'D' : ' ');
            break;
        case 7:
    ...
    const char *sbuf, void *val, uint32_t size)
{
    ...
    else
    {
        ...
        rc = _bdmf_configure(mo, aggr->fields, (char *) sbuf);
    ...
    const bdmf_attr_enum_table_t *enum_table, const char *sbuf, bdmf_number *pmask)
{
    ...
    do
    {
        ...
        if (bdmf_attr_get_enum_val_hlp(enum_table, valname, &eval))
            BDMF_TRACE_RET_OBJ(BDMF_ERR_PARSE, mo, "Can't translate %s to rdpa_if\n", valname);
...
int bdmf_attr_make(struct bdmf_type *drv, struct bdmf_attr *attr)
{
    ...
    if ((attr->type == bdmf_attr_enum || attr->type == bdmf_attr_enum_mask) &&
         (!attr->ts.enum_table || !attr->ts.enum_table->values))
...
int bdmf_mattr_set(bdmf_object_handle mo, const bdmf_mattr_handle hmattr)
{
        ...
        if (mo->drv->aattr)
        {
            ...
            while (attr && attr->name)
            {
                if ((attr->flags & BDMF_ATTR_MANDATORY))
                {
                    ...
                    if (!entry)
                    {
                        BDMF_TRACE_RET_OBJ(BDMF_ERR_PARM, mo, "mandatory attribute(s) not set: %s\n",
                            attr->name);
                    }
                }
                ++attr;
    ...
    else
    {
        ...
        if (!roll_back_buf)
            BDMF_TRACE_RET_OBJ(BDMF_ERR_NOMEM, mo, "mattr_set: can't allocate roll-back buffer\n");
        bdmf_lock();
    }
