...
struct bdmf_attr
{
    ...
    int (*delete)(struct bdmf_object *mo, struct bdmf_attr *ad, bdmf_index index);
...
struct bdmf_type
{
    ...
    */
    ...
    int (*link_down)(struct bdmf_object *this, struct bdmf_object *mo_downstream,
        const char *link_attrs);

    /** Called when other object is linked on top of THIS object */
    int (*link_up)(struct bdmf_object *this, struct bdmf_object *mo_upstream,
        const char *link_attrs);

    /** Called when object's downlink is disconnected */
    void (*unlink_down)(struct bdmf_object *this, struct bdmf_object *mo_downstream);

    /** Called when object's uplink is disconnected */
    void (*unlink_up)(struct bdmf_object *this, struct bdmf_object *mo_upstream);
 ...
 * \param[in]   decode  Decoding function
 * \return 0=OK or error code <0
 */
 ...
 * \param[out]  s       Decoded string
 * \return 0=OK or error code <0
 */
int bdmf_history_decode_string(char **s);

/** Decode binary argument
 * \param[out]  buf     Buffer for the data
 * \param[out]  size    Buffer size
 * \return >=OK-number of bytes written into buf.\n
 * or error code <0
 */
 ...
 * \return 0=OK or error code <0
 */
int bdmf_history_decode_rc(int *rc);
...
static inline int bdmf_history_play(const char *fname)
{
    return BDMF_ERR_NOT_SUPPORTED;
}
