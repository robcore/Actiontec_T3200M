...
static void hs_uart_dma_tx_cb(void *data)
{
   ...
   if( (HS_UART_REG(dma_request_desc->port))->LCR & HS_UART_LCR_SLIP )
   {
      /* Wait for TX FIFO Empty. FIXME: This could be overkill, sim and rtl indicates this should be done */
      while( !((HS_UART_REG(dma_request_desc->port))->LSR & HS_UART_LSR_TX_IDLE) 
           || ((HS_UART_REG(dma_request_desc->port))->LSR & HS_UART_LSR_TX_DATA_AVAIL) );
      
      /* Wait for TX_PACKET_READY to be 1 */
      while( !((HS_UART_REG(dma_request_desc->port))->LSR & HS_UART_LSR_TX_PACKET_RDY) );
      
      /* Indicate end of TX packet */
      (HS_UART_REG(dma_request_desc->port))->FCR &= ~HS_UART_FCR_TX_PACKET;            
   }     
...
int hs_uart_dma_init( dma_complete_callback rx_callback, dma_complete_callback tx_callback )
{
   int ret = -ENOMEM;
...
// int hs_uart_issue_tx_dma( dma_addr_t dma_addr, unsigned int length )
// {
//    struct dma_xfer_desc
//    struct dma_async_tx_descriptor *desc;
//    struct scatterlist sg;
...
//    desc = dmaengine_prep_slave_sg(hs_uart_dma_cfg.tx_dma_chan, &sg, 1, 
//          hs_uart_dma_cfg.tx_slave_cfg.direction, 0);
//          
//    if (!desc) {
//       printk(KERN_ERR "Couldn't prepare DMA slave!\n");
//       return -ENXIO;
...
// 
//    /* Set callback */
//    desc->callback = hs_uart_dma_tx_cb;
...
// }
// 
// int hs_uart_issue_rx_dma( dma_addr_t dma_addr, unsigned int length )
// {
//    struct dma_async_tx_descriptor *desc;
//    struct scatterlist sg;
//    
...
//           xfer_len, offset_in_page(dma_addr));
//    sg_dma_address(&sg) = dma_addr;
//    desc = dmaengine_prep_slave_sg(hs_uart_dma_cfg.rx_dma_chan, &sg, 1, 
//          hs_uart_dma_cfg.rx_slave_cfg.direction, 0);
//          
//    if (!desc) {
//       printk(KERN_ERR "Couldn't prepare DMA slave!\n");
//       return -ENXIO;
...
// 
//    /* Set callback */
//    desc->callback = hs_uart_dma_rx_cb;
//    desc->callback_param = dma_addr;
// 
//    /* Submit DMA */
//    dmaengine_submit((struct dma_async_tx_descriptor *)desc);
