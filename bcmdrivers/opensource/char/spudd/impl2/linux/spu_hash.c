...
struct ahash_alg spu_md5_alg = {
	...
	/*.statesize	=	sizeof(struct md5_state),*/ /*TODO: */
	.base = {
	    .cra_name		=	"hmac(md5)",
...
int spu_hash_register(void)
{
    ...
    if(rc)
    {
        printk(KERN_ERR "alg hmac(md5) registration failed with error %d\n", rc);
    }
    ...
    if(rc)
    {
        printk(KERN_ERR "alg hmac(sha1) registration failed with error %d\n", rc);
    }
...
int spu_hash_unregister(void)
{
    ...
    rc = crypto_unregister_ahash(&spu_md5_alg);
    if(rc)
    {
        printk(KERN_ERR "alg hmac(md5) unregistration failed with error %d\n", rc);
    }

    rc = crypto_unregister_ahash(&spu_sha1_alg);
    if(rc)
    {
        printk(KERN_ERR "alg hmac(sha1) unregistration failed with error %d\n", rc);
    }
