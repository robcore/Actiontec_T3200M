...
static int doReadBuffer(PBP_SPISLAVE_INFO pSpiDev, unsigned long addr, unsigned long *data, unsigned long len)
{
    uint8_t buf[12];
    int status;   
...
static int doWriteBuffer(PBP_SPISLAVE_INFO pSpiDev, unsigned long addr, unsigned long *data, unsigned long len)
{
   uint8_t buf[512]; 
   int status;
   ...
   {
      printk(KERN_ERR "SPI Slave Write: write to addr:0x%lx failed.  Len (%ld) too long.\n", addr, len);
      return(-1);
   }
...
int kerSysBcmSpiSlaveModify(int dev, unsigned long addr, unsigned long data, unsigned long mask, unsigned long len)
{
   PBP_SPISLAVE_INFO pSpiDev;
