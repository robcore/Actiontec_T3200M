...
//}}}
//{{{ proc entry
static int _proc_write(struct file *f, const char *buf, unsigned long cnt, void *data);
...
static int _proc_read(char *buf, char **start, off_t off, int cnt, int *eof, void *data)
{
    int r = 0;

    r += sprintf(buf + r, "Usage: echo command > /proc/" XDSL_DISTPOINT_PROC_ENTRY_NAME "\n\n");
    r += sprintf(buf + r, "supported commands: \n");
    r += sprintf(buf + r, "    reset <enter|exit> <name|id>\n");
    r += sprintf(buf + r, "    gpio [<id> <value>]\n");
    r += sprintf(buf + r, "    status\n");
...
static int _proc_control_reset(char * cmd, char * id_or_name)
{
    ...
    // search by id
    if (0 == strict_strtoul(id_or_name, 10, &j)) {
    ...
    if (0 == strcmp(arg[0], "reset")) {
        ...
        switch (argc) {
            case 1:
                ...
                break;
            case 3:
                if (strict_strtoul(arg[1], 10, &gpio_num)) {
                    buffer_len = sprintf(buffer, "invalid gpio id %s", arg[1]);
                    mtty_writeln_buffer();
                    return -EFAULT;
                }
                if (strict_strtoul(arg[2], 10, &gpio_value)) {
...
static int _create_xdsl_distpoint_proc_entry(void)
{
    ...
    p0 = create_proc_entry(XDSL_DISTPOINT_PROC_ENTRY_NAME, 0644, NULL);
    if (p0 == NULL) {
        printk(KERN_WARNING "Failed to create xdsl_distpoint proc entry.\n");
        return -1;
    }
    p0->read_proc = _proc_read;
    p0->write_proc = _proc_write;
