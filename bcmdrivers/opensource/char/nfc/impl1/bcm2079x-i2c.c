				 ...
				 size_t count, loff_t *offset)
{
	...
	// bcm63000_xfer fifo limits "packets" to 32-bytes?
	...
	if (kern_buf[0] == PACKET_TYPE_NCI)
		pkt_len = PACKET_HEADER_SIZE_NCI + kern_buf[PACKET_HEADER_SIZE_NCI - 1];
	else if (kern_buf[0] == PACKET_TYPE_HCIEV)
					...
					unsigned int cmd, unsigned long arg)
{
	...
	case BCMNFC_POWER_CTL:
		if (bcm2079x_dev->pdata.power_gpio == BP_NOT_DEFINED)
			return -ENODEV;
		kerSysSetGpioState(bcm2079x_dev->pdata.power_gpio,
				arg ? kGpioActive : kGpioInactive);
		return 0;
	case BCMNFC_WAKE_CTL:
		if (bcm2079x_dev->pdata.wake_gpio == BP_NOT_DEFINED)
			return -ENODEV;
		kerSysSetGpioState(bcm2079x_dev->pdata.wake_gpio,
				arg ? kGpioInactive : kGpioActive);
		return 0;
...
static int bcm2079x_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	rc = BcmHalMapInterruptEx((FN_HANDLER)bcm2079x_dev_irq_handler,
		(unsigned)bcm2079x_dev, client->irq, bcm2079x_id[0].name,
