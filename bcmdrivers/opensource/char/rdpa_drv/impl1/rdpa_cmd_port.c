...
int rdpa_cmd_port_ioctl(unsigned long arg)
{
    ...
    if (port.is_wan_side)
    {
        if ((rdpa_if_wan0 + port.port_idx) > rdpa_if_wan1)
        {
            printk("Invalid Wan Port \n");
            return BDMF_ERR_PARM;
        }
        port_Id = rdpa_if_wan0 + port.port_idx;
    }
    else
    {
        if ((rdpa_if_lan0 + port.port_idx) > rdpa_if_lan_max)
        {
            printk("Invalid Uni Port \n");
            return BDMF_ERR_PARM;
        }
        port_Id = port.port_idx + rdpa_if_lan0;
    }
    
    bdmf_lock();

    rc = rdpa_port_get( port_Id, &port_obj );
    if (rc)
    {
        bdmf_unlock();
        return rc;
    ...
    switch(port.cmd)
    {
        ...
        case RDPA_IOCTL_PORT_CMD_SAL_MISS_ACTION_GET: 
        {
        ...
        case RDPA_IOCTL_PORT_CMD_SAL_MISS_ACTION_SET: 
        {
            rc = rdpa_port_cfg_get(port_obj, &cfg);
            if (!rc)
            {
        ...
        case RDPA_IOCTL_PORT_CMD_DAL_MISS_ACTION_GET: 
        {
            rc = rdpa_port_cfg_get(port_obj, &cfg);
            if (!rc)
            {
        ...
        case RDPA_IOCTL_PORT_CMD_DAL_MISS_ACTION_SET: 
        {
            rc = rdpa_port_cfg_get(port_obj, &cfg);
            if (!rc)
            {
