...
static Bool epon_mode_get(rdpa_epon_mode *mode)
{
    ...
    if (!get_rdpa_wan_type(&wan_type) && wan_type == rdpa_wan_epon)
    {
        if (rdpa_epon_get(&epon_obj))
        {
            CMD_TM_LOG_ERROR("rdpa_epon_get failed\n");
        }
        else
        {
            if (rdpa_epon_mode_get(epon_obj, mode))
                CMD_TM_LOG_ERROR("rdpa_epon_mode_get failed\n");
            else
                ret = TRUE;
            bdmf_put(epon_obj);
        }
...
#if defined(CONFIG_BCM96838) || defined(CONFIG_BCM96848)
        rdpa_epon_mode epon_mode = rdpa_epon_ctc;
        
        if (epon_mode_get(&epon_mode) &&
            (epon_mode == rdpa_epon_bcm || epon_mode == rdpa_epon_dpoe))
        {
...
static int set_q_drop_alg(rdpa_drv_ioctl_tm_t *ptm)
{
    ...
    queue_cfg.red_high_threshold = ptm->red_high_thr;
    queue_cfg.red_low_threshold  = ptm->red_low_thr;

    CMD_TM_LOG_DEBUG("rdpa_egress_tm_queue_cfg_set(): "
                     "tm(%u) q_id(%u) drop_alg(%u) hi_thr(%u) lo_thr(%u)",
                     ptm->tm_id, queue_cfg.queue_id, queue_cfg.drop_alg,
                     queue_cfg.red_high_threshold, queue_cfg.red_low_threshold);
