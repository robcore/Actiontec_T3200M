...
typedef union
{
   struct {
      ...
      uint8_t     auth_key[RDD_IPSEC_DS_SA_DESC_AUTH_KEY_NUMBER];
      uint8_t     crypt_key[RDD_IPSEC_DS_SA_DESC_CRYPT_KEY_NUMBER];
   };

   RDD_IPSEC_DS_SA_DESC_DTS sa_desc;
                                      ...
                                      uint32_t *crypt_config2)
{
   ...
   memset((void *)&sa_desc, 0, sizeof(RDD_IPSEC_DS_SA_DESC_DTS));

   /* format auth config register */
   RDD_IPSEC_DS_SA_DESC_ENG_CFG_WRITE(0, &sa_desc);
   RDD_IPSEC_DS_SA_DESC_HMAC_DIS_WRITE(0, &sa_desc);
   RDD_IPSEC_DS_SA_DESC_AUTH_KEY_FETCH_DIS_WRITE(0, &sa_desc);
   RDD_IPSEC_DS_SA_DESC_AUTH_ALG_WRITE(auth_alg, &sa_desc);

   /* format crypt config register */   
   RDD_IPSEC_DS_SA_DESC_NXT_HDR_WRITE(nxt_hdr, &sa_desc);
   RDD_IPSEC_DS_SA_DESC_AES_KEY_SIZE_WRITE(aes_key_size, &sa_desc);
   RDD_IPSEC_DS_SA_DESC_DES_DEC_VEC_WRITE(des_dec_vec, &sa_desc);
   RDD_IPSEC_DS_SA_DESC_DES_ITERS_WRITE(des_iters, &sa_desc);
   RDD_IPSEC_DS_SA_DESC_RNG_CLK_EN_WRITE(rng_clk_en, &sa_desc);
   RDD_IPSEC_DS_SA_DESC_RNG_SEED_WRITE(0, &sa_desc);
   RDD_IPSEC_DS_SA_DESC_RNG_SAMPLE_NUM_WRITE(rng_sample_num, &sa_desc);
   RDD_IPSEC_DS_SA_DESC_CRYPT_KEY_FETCH_DIS_WRITE(0, &sa_desc);
   RDD_IPSEC_DS_SA_DESC_WRITE_DIS_WRITE(0, &sa_desc);
   RDD_IPSEC_DS_SA_DESC_MECH_WRITE(mech, &sa_desc);
   if (dir == rdpa_dir_ds)
      RDD_IPSEC_DS_SA_DESC_DECRYPT_WRITE(IPSEC_RDB_DECRYPT, &sa_desc);
   else
      RDD_IPSEC_DS_SA_DESC_DECRYPT_WRITE(IPSEC_RDB_ENCRYPT_DEFAULT, &sa_desc);
   RDD_IPSEC_DS_SA_DESC_CRYPT_ALG_WRITE(crypt_alg, &sa_desc);

   /* format crypt config 2 register */   
   RDD_IPSEC_DS_SA_DESC_CLUSTR_OVRRD_WRITE(0, &sa_desc);
   RDD_IPSEC_DS_SA_DESC_READ_CLUSTR_SEL_WRITE(0, &sa_desc);
   RDD_IPSEC_DS_SA_DESC_WRITE_CLUSTR_SEL_WRITE(0, &sa_desc);
   RDD_IPSEC_DS_SA_DESC_ADD_WRITE(0, &sa_desc);
