...
typedef enum {
    ...
    rdpa_epon_attr_init = 1, /* init : W : bool/1 : one time EPON stack init */
    rdpa_epon_attr_max_link_count = 2, /* max_link_count : RW : number/1 : how many links can be used */
    rdpa_epon_attr_registered_links = 3, /* registered_links : R : number/1 : how many links are registered */
    rdpa_epon_attr_laser_tx_mode = 4, /* laser_tx_mode : RW : enum/4 : Set the laser TX mode */
    rdpa_epon_attr_laser_rx_enable = 5, /* laser_rx_enable : RW : bool/1 : Set the Rx laser enable */
    rdpa_epon_attr_holdover = 6, /* holdover : RW : aggregate/8 epon_holdover(rdpa_epon_holdover_t) : EPON holdover properties */
    rdpa_epon_attr_los_threshold = 7, /* los_threshold : RW : aggregate/4 epon_los(rdpa_epon_los_t) : PON and Gate LOS time threshold        before any action is taken */
    rdpa_epon_attr_link_enable = 8, /* link_enable : RWF : bool/1[8] : Enable epon link */
    rdpa_epon_attr_link_mpcp_state = 9, /* link_mpcp_state : RF : enum/4[8] : Get the link MPCP regitration state */
    rdpa_epon_attr_mcast_link = 10, /* mcast_link : RWF : aggregate/5[8] epon_mcast_link(rdpa_epon_mcast_link_t) : EPON multicast link properties */
    rdpa_epon_attr_fec_enable = 11, /* fec_enable : RWF : aggregate/2[8] epon_fec_enable(rdpa_epon_fec_enable_t) : enable EPON MAC FEC for us/ds or both */
    rdpa_epon_attr_burst_cap_map = 12, /* burst_cap_map : RWF : aggregate/16[8] burst_cap_per_priority(rdpa_epon_burst_cap_per_priority_t) : all links burst cap per pri */
