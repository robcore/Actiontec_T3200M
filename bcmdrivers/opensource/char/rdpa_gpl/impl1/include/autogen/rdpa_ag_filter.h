...
typedef enum {
    rdpa_filter_attr_global_cfg = 0, /* global_cfg : RW : aggregate/1 filter_global_cfg(rdpa_filter_global_cfg_t) : Global configuration */
    rdpa_filter_attr_etype_udef = 1, /* etype_udef : RWF : number/2[4] : Ether-Type, User-Defined filter */
    rdpa_filter_attr_oui_val = 2, /* oui_val : RWF : number/4[196(filter_oui_val_key)] : MAC Address OUI filter, Value */
    rdpa_filter_attr_tpid_vals = 3, /* tpid_vals : RW : aggregate/4 filter_tpid_vals(rdpa_filter_tpid_vals_t) : TPID filter, Values */
    rdpa_filter_attr_entry = 4, /* entry : RWF : aggregate/8[1029(filter_key)] filter_ctrl(rdpa_filter_ctrl_t) : Entry - Filter on a Port */
    rdpa_filter_attr_stats = 5, /* stats : RF : number/2[42(filter_stats_key)] : Drop statistics */
...
/** Get filter/entry attribute entry.
 *
 * Get Entry - Filter on a Port.
 * \param[in]   mo_ filter object handle or mattr transaction handle
 * \param[in]   ai_ Attribute array index
 ...
 * The function can be called in task context only.
 */
static inline int rdpa_filter_entry_get(bdmf_object_handle mo_, rdpa_filter_key_t * ai_, rdpa_filter_ctrl_t * entry_)
{
    return bdmf_attrelem_get_as_buf(mo_, rdpa_filter_attr_entry, (bdmf_index)ai_, entry_, sizeof(*entry_));
}


/** Set filter/entry attribute entry.
 *
 * Set Entry - Filter on a Port.
 * \param[in]   mo_ filter object handle or mattr transaction handle
 * \param[in]   ai_ Attribute array index
 ...
 * The function can be called in task context only.
 */
static inline int rdpa_filter_entry_set(bdmf_object_handle mo_, rdpa_filter_key_t * ai_, const rdpa_filter_ctrl_t * entry_)
{
    return bdmf_attrelem_set_as_buf(mo_, rdpa_filter_attr_entry, (bdmf_index)ai_, entry_, sizeof(*entry_));
}


/** Get next filter/entry attribute entry.
 *
 * Get next Entry - Filter on a Port.
 * \param[in]   mo_ filter object handle or mattr transaction handle
 * \param[in,out]   ai_ Attribute array index
 * \return 0 or error code < 0
 * The function can be called in task context only.
 */
static inline int rdpa_filter_entry_get_next(bdmf_object_handle mo_, rdpa_filter_key_t * ai_)
{
    return bdmf_attrelem_get_next(mo_, rdpa_filter_attr_entry, (bdmf_index *)ai_);
}
