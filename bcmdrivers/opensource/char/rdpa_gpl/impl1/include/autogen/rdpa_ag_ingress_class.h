...
typedef enum {
    rdpa_ingress_class_attr_dir = 0, /* dir : MKRI : enum/4 : Traffic Direction */
    rdpa_ingress_class_attr_index = 1, /* index : KRI : number/4 : Ingress class index */
    rdpa_ingress_class_attr_cfg = 2, /* cfg : MRI : aggregate/48 class_configuration(rdpa_ic_cfg_t ) : Ingress class configuration */
    rdpa_ingress_class_attr_nflow = 3, /* nflow : R : number/4 : Number of associated classification flows */
    rdpa_ingress_class_attr_flow = 4, /* flow : RWADF : aggregate/152[255] class_info(rdpa_ic_info_t ) : Ingress class flow entry */
    rdpa_ingress_class_attr_flow_stat = 5, /* flow_stat : RF : aggregate/8[255] rdpa_stat(rdpa_stat_t) : Ingress class flow statistics (can be enabled in system object) */
    rdpa_ingress_class_attr_port_action = 6, /* port_action : RWF : aggregate/5[254(port_action_key)] port_action(rdpa_port_action_t) : Per egress port action */
    rdpa_ingress_class_attr_flush = 7, /* flush : W : bool/1 : Flush ingress class table (remove all configured flows) */
