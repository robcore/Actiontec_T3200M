...
#ifdef CONFIG_BLOG
static int bcmxtmrt_get_xmit_channel(void *dev_p, int channel, unsigned uMark);
#endif
...
#if defined(GPL_CODE)
extern unsigned short inet_green_led;        
extern unsigned short inet_red_led;
#endif

#if defined(GPL_CODE_SMARTLED) 
#if defined(GPL_CODE_63168_CHIP)
extern UBOOL8 AEI_have_packets_received(void);
#endif
#endif
   ...
   do
   {
...
#endif
      {
         ...
         else if (!ulCell) /* process packet, pDev != NULL */
         {
            ...
            bcmxtmrt_processRxPkt(pDevCtx, rxdma, (pNBuff_t)pBuf, 0,
                                  dmaDesc.status, dmaDesc.length);
                         ...
                         int *eof, void *data)
{
   ...
   for (i = 0; i < MAX_DEV_CTXS; i++)
   {
      ...
      if (pDevCtx != (PBCMXTMRT_DEV_CONTEXT) NULL)
      {
         for (j = 0; j < pDevCtx->ulTxQInfosSize; j++)
         {
            ...
            sz += sprintf(page + sz,
                          "BD RingOffset: 0x%08lx, Word1: 0x%08lx\n",                  
...
#if !(defined(CONFIG_BCM_BPM) || defined(CONFIG_BCM_BPM_MODULE))
            sz += sprintf(page + sz,
                          "%s tx_chan_size: %lu, tx_chan_filled: %lu\n",
...
#if !(defined(CONFIG_BCM_BPM) || defined(CONFIG_BCM_BPM_MODULE))
            sz += sprintf(page + sz,
                          "tx_chan:%d tx_chan_size: %lu, tx_chan_filled: %lu\n",
   ...
   sz += sprintf(page + sz,
                 "queue_condition: %lu %lu %lu, drop_condition: %lu %lu %lu\n\n",
                         ...
                         int *eof, void *data)
{
    ...
    for (i = 0; i < MAX_RECEIVE_QUEUES; i++)
    {
       ...
       if (rxdma != NULL)
       {
          ...
          sz += sprintf(page + sz,
                        "DMA cfg: 0x%08lx, intstat: 0x%08lx, intmask: 0x%08lx\n",
...
int bcmxapi_GetPaddingAdjustedLen (int len)
{
   return (bcmxapi_get_pad_len (len));
