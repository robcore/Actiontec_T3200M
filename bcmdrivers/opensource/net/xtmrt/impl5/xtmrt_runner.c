...
unsigned int gulXtmRdpPadWorkaround = 0 ; /* Disabled in CPU */
unsigned int gulXtmRdpAlignWorkaround = 0 ; /* Disabled in CPU */
unsigned int gulXtmRdpHdRoomWorkaround = 0 ; /* Enabled for 63138 only */
...
int bcmxapi_module_init(void)
{
   ...
   printk ("\nbcmxtmrt :- gulXtmRdpPadWorkaround = %d, &gulXtmRdpPadWorkaround = %x \n",
            (int) gulXtmRdpPadWorkaround, (unsigned int) &gulXtmRdpPadWorkaround) ;

   printk ("\nbcmxtmrt :- gulXtmRdpAlignWorkaround = %d, &gulXtmRdpAlignWorkaround = %x \n",
            (int) gulXtmRdpAlignWorkaround, (unsigned int) &gulXtmRdpAlignWorkaround) ;

#if defined (CONFIG_BCM963138)
   gulXtmRdpHdRoomWorkaround = 0x1 ; /* as RDP aligns/pads to 16-byte boundary with minimum padding. All the packets that go to DSL WAN US thru RDP will be
                                      * subjected to this functionality. RDP
                                      * assumes there is enough headroom for effecting the alignment.
                                      */
#endif

   printk ("\nbcmxtmrt :- gulXtmRdpHdRoomWorkaround = %d, &gulXtmRdpHdRoomWorkaround = %x \n",
            (int) gulXtmRdpHdRoomWorkaround, (unsigned int) &gulXtmRdpHdRoomWorkaround) ;
                         ...
                         PXTMRT_TRANSMIT_QUEUE_ID pTxQId)
{
   ...
   if ((ulPort < MAX_PHY_PORTS) && (pTxQId->ulSubPriority < MAX_SUB_PRIORITIES))
   {
       ...
       */
//      txdma->ulQueueSize   = pTxQId->ulQueueSize; // pGi->ulNumExtBufs;
      ...
      if (cfg_xtmflows(pTxQId->ulQueueIndex, pDevCtx->ulTrafficType, pDevCtx->ulHdrType,
      ...
      if (egress_tm != NULL) {
         ...
         queue_cfg.drop_threshold     = bcmxapi_getTxQThreshold () ;
         ...
         {
            printk(CARDNAME "SetTxQ: rdpa_egress_tm_queue_cfg_set error rc=%d\n", rc);
         }
      } /* if (egress_tm) */
   }
                             ...
                             volatile BcmPktDma_XtmTxDma *txdma)
{
   ...
   if (egress_tm != NULL)
   {
      ...
      if ((rc = rdpa_egress_tm_queue_cfg_set(egress_tm, 0, &queue_cfg)))
      {
         printk(CARDNAME ": rdpa_egress_tm_queue_cfg_set error rc=%d\n", rc);
      }
   }
...
#if defined(CONFIG_BCM963138)
   aLen = bcmxapi_get_pad_len (len);
#else
   aLen = len ;
#endif
   return (aLen) ;
}  /* bcmxapi_GetPaddingAdjustedLen () */
