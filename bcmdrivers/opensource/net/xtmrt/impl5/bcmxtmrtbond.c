...
#if (!defined(FAP_4KE) && !(defined(CONFIG_BCM_FAP) || defined(CONFIG_BCM_FAP_MODULE))) && defined (SUPPORT_631XX_TX_RX_IUDMA)
 ...
 * For Runner based platforms, Runner does not do PTM bonding functionality
 * yet. Until it occurs, the Path for DSL would be through CPU.
 */
...
int bcmxtmrt_ptmbond_add_hdr(UINT32 ulTxPafEnabled, pNBuff_t *ppNBuff, UINT32 ulPtmPrioIdx)
{
   ...
   if (IS_SKBUFF_PTR(*ppNBuff))
   {
      ...
      minheadroom += (int)(skb->data - minheadroom) & (L1_CACHE_BYTES-1);
#endif
      ...
      if (skb && skb_headroom(skb) >= minheadroom)
      {
          ...
          */
         pData = skb_push(skb, minheadroom);
         frags = 1;
#else
         //printk ("skb->len = %d\n", skb->len);
         adjustLen = bcmxapi_GetPaddingAdjustedLen (skb->len) ;
         if (!ulTxPafEnabled)
            frags = constructPtmBondHdr(pPtmSrcBondHdr, ulPtmPrioIdx, adjustLen) ;
         else {
#if defined(SUPPORT_631XX_TX_RX_IUDMA)
            frags = constructPtmBondHdrTxPAF(pPtmSrcBondHdr, ulPtmPrioIdx, adjustLen) ;
#else
             ...
             */
            cache_flush_len(skb->data - minheadroom, L1_CACHE_BYTES);
            frags = 1 ;
			...
			if (frags)
         {
				...
				pData = skb_push(skb, minheadroom);
				PMON_US_LOG(3);
                                if (!((uint32)(pData) & 0x1))  /* 16-bit aligned packet boundary */
...
#if !defined(SUPPORT_631XX_TX_RX_IUDMA) && !defined(CONFIG_BCM963268)
_skipFragmentationSkb :
   ...
   else
   {
      ...
      /*Align required headroom up to the nearest cache line to ensure that we
       *don't have to share this cache line with other SW.*/
      minheadroom += (int)(fkb_data(fkb) - minheadroom) & (L1_CACHE_BYTES-1);
#endif
      ...
      if (headroom >= minheadroom)
      {
          ...
          * will be added in by FAP 4KE.
          */
         pData = fkb_push(fkb, minheadroom);
         frags = 1;
#else
         //printk ("fkb->len = %d\n", fkb->len);
         adjustLen = bcmxapi_GetPaddingAdjustedLen (fkb->len) ;
         if (!ulTxPafEnabled)
            frags = constructPtmBondHdr(pPtmSrcBondHdr, ulPtmPrioIdx, adjustLen) ;
         else {
#if defined(SUPPORT_631XX_TX_RX_IUDMA)
             ...
             *undetermined later time, after RDP FW has written the PTM bonding header into it.
             */
            cache_flush_len(fkb_data(fkb) - minheadroom, L1_CACHE_BYTES);
            frags = 1 ;
#if !defined(CONFIG_BCM963268)
