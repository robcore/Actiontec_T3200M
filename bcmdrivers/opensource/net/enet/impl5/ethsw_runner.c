...
#endif
#if defined(GPL_CODE_63138_CHIP) && defined(GPL_CODE)
...
extern UBOOL8 g_wan_eth_link_up;
extern UBOOL8 g_wan_sfp_link_up;
#endif
#if defined(GPL_CODE_SCKIPIO_SUPPORT)
...
#endif
        {
#if 0 // defined(GPL_CODE) && defined(CONFIG_I2C)
...
#if defined(GPL_CODE_MUTIL_WAN_ETHERNET_PHY)
    if (g_wan_eth_link_up) // Because of WAN ETH and SFP share 1 phy port on crossbar.
        sfp_status = SFP_MODULE_OUT;
#endif
...
sfp_module_out:
#if defined(GPL_CODE_MUTIL_WAN_ETHERNET_PHY)
            if(sfp_status == SFP_MODULE_OUT && !g_wan_eth_link_up && ethsw_sfp_module_detect(phyId))
#else
...
#if defined(GPL_CODE_63138_CHIP) && defined(GPL_CODE)
#if defined(GPL_CODE_SFP_LED)
            bcmenet_set_eth_sfp_led(100);
#endif
#if defined(GPL_CODE_MUTIL_WAN_ETHERNET_PHY)
            g_wan_sfp_link_up = TRUE;
#endif
#endif
            goto sfp_end;
...
{
#if defined(GPL_CODE_63138_WAN_ETHER_LINK_PATCH)
...
#endif
#if defined(GPL_CODE_63138_LAN_ETHER_LINK_PATCH)
    {
        //enable ethernet@wirespeed of GE port
        uint16 v16;
