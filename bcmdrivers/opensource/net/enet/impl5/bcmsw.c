...
#if defined(GPL_CODE_ENABLE_PAUSE)
void sf2_conf_flow_control(void);
#endif
#if defined(GPL_CODE_EXT_SWITCH_PORT_ISOLATE)
void conf_pbvlan_init(void);
#endif
...
int remove_arl_entry_ext(uint8_t *mac)
{
    ...
    /* 63138 switch chip ID == 0, so add a judgment to support it */
		    && extSwId != 0
#endif
		    )
    {
...
#else
            extsw_wreg_wrap(PAGE_AVTBL_ACCESS, REG_ARL_MAC_INDX_LO, (uint8_t *)&mac_vid_v64[2], 6);
#endif
...
#if defined(GPL_CODE_HPNA)
            /* set the SF2 RGMII control register for HPNA,
             * for 963138 the RGMII port is 12 */
            if ((phy_id & BCM_PHY_ID_M) == HPNA_PHY_ID)
            {
                sw_rgmii_ctrl_reg = (void *)(SWITCH_BASE + rgmii_port_reg_offset[PHY_PORT_IDX(phy_port)].rgmii_ctrl);
                val32 = 0;

                val32 = (SF2_ENABLE_PORT_RGMII_INTF | SF2_RGMII_PORT_MODE_EXT_MII);
...
int sf2_port_erc_config(struct ethswctl_data *e)
{
    ...
    if (e->type == TYPE_SET) {
        ...
        /* configure shaper rate limit*/
        val32 = e->limit & SHAPER_RATE_BURST_VAL_MASK;
        page = q_shaper? PAGE_Q0_EGRESS_SHAPER + e->queue:
                                    PAGE_PORT_EGRESS_SHAPER;
        reg  = e->sub_type == SHAPER_PACKET_MODE? SF2_REG_PN_SHAPER_RATE_PKT:
                                                  SF2_REG_PN_SHAPER_RATE_BYTE;
        extsw_wreg_wrap(page, reg + e->port * 4, &val32, 4);

        /* configure shaper burst size */
        val32 = e->burst_size & SHAPER_RATE_BURST_VAL_MASK;
        reg  = e->sub_type == SHAPER_PACKET_MODE? SF2_REG_PN_SHAPER_BURST_SZ_PKT:
        ...
        val16 &= ~(1 << e->port);
        val16 |= e->sub_type == SHAPER_PACKET_MODE? (1 << e->port): 0;
...
int bcmeapi_ioctl_que_map(BcmEnet_devctrl *pDevCtrl, struct ethswctl_data *e)
{
    ...
    copy_to_user((void *)&e->val, (void *)&wanQueMap, sizeof(e->val));
    copy_to_user((void *)&e->priority, (void *)&queRemap, sizeof(e->priority));
    return 0;
}


/*
 * Enable Tx and Rx pause Frames on all ports on External Switch.
 */
#if defined(GPL_CODE_ENABLE_PAUSE)
void sf2_conf_flow_control(void)
{
#define PORT_TX_PAUSE_ON  0xBF
#define PORT_RX_PAUSE_ON  0xBF << TOTAL_SWITCH_PORTS
    uint32_t val;

    /* Enable Tx/Rx Pause Frames on Switch Port 0 - 5, Port 7 and Port 8 */
    extsw_rreg_wrap(PAGE_CONTROL, REG_PAUSE_CAPBILITY, (uint8_t *)&val, 4);

    val |= (PORT_TX_PAUSE_ON | PORT_RX_PAUSE_ON | REG_PAUSE_CAPBILITY_OVERRIDE);

    extsw_wreg_wrap(PAGE_CONTROL, REG_PAUSE_CAPBILITY, (uint8_t *)&val, 4);
...
int bcmeapi_ioctl_extsw_port_storm_ctrl(struct ethswctl_data *e)
{
    ...
    if (e->type == TYPE_SET) {        
        ...
        val32 = REG_PN_BUCK0_ENABLE_MASK | REG_PN_BUCK0_MODE_MASK | REG_PN_BUCK1_MODE_MASK; // use bucket 0
...
#if defined(GPL_CODE_EXT_SWITCH_PORT_ISOLATE)
static uint16_t g_isolated_portmask = 0;
static uint32_t g_enable_isolated_port = 0;

uint16_t get_isolated_portmask(void)
{
    return g_isolated_portmask;
}

uint32_t is_enable_isolated_port(void)
{
    return g_enable_isolated_port;
}

/*
 * enable/disable aei port isolated for switch
 */
#define EXTSW_UNIT                      1
void conf_isolated_port(int enable)
{
    int port;
    uint16_t vlan_map;
    unsigned long port_map = enet_get_portmap(EXTSW_UNIT);

    if (!g_isolated_portmask)
        return;

    g_enable_isolated_port = enable;
    for (port = 0; port <= MAX_SWITCH_PORTS; port++)
    {
        if (!(port_map & (1 << port)))
            continue;

        vlan_map = extsw_get_pbvlan(port);
        if (enable) {
            if (g_isolated_portmask & (1 << port))
                vlan_map = g_isolated_portmask | (1 << MIPS_PORT_ID);
            else
                vlan_map &= ~g_isolated_portmask;
        } else {
            if (g_isolated_portmask & (1 << port))
                vlan_map =  (uint16_t)port_map | (1 << MIPS_PORT_ID);
            else
                vlan_map |= g_isolated_portmask;
        }
        printk("Port based VLAN mapping for port %d set to 0x%x\n", port, vlan_map);
        extsw_set_pbvlan(port, vlan_map);
    }
}
...
void conf_pbvlan_init(void)
{
    ...
    for(port = 0; port < MAX_SWITCH_PORTS; port++)
    {
        ...
        port_flags = enet_get_port_flags(EXTSW_UNIT, port);

        if (IsPortToBeIsolatedAtInit(port_flags))
            isolated_portmask |= 1 << port;
    }

    g_isolated_portmask = isolated_portmask;
    if (isolated_portmask)
        conf_isolated_port(1);
}
