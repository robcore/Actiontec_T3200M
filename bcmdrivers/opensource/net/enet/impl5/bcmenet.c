...
#if defined(GPL_CODE_SUPPORT_SCKIPIO_GFAST)
extern int g_sfp_flag;
#endif

#if defined(GPL_CODE_SUPPORT_100M_ETH_RELINK)
UBOOL8 relinked[MAX_TOTAL_SWITCH_PORTS] =  {[0 ... (MAX_TOTAL_SWITCH_PORTS-1)] = FALSE};

/* Can probably do a better job parsing the boardparms.c setup and extracting the port
 * but for now, just do this macro to read the cabled ports...
 */
#define is_cabled_eth_port(unit,port) ((unit == 0 && port == 0) || \
                                       (unit == 1 && port >=8 && port <=11 ))
#endif
#if defined(GPL_CODE_MUTIL_WAN_ETHERNET_PHY)
UBOOL8 g_wan_eth_link_up = FALSE;
UBOOL8 g_wan_sfp_link_up = FALSE;
#endif

#if defined(GPL_CODE_SW_WAN_ETH_LED) || defined(GPL_CODE_INET_LED_BLINK)
int enetWanLedOn=0;
#if defined(GPL_CODE_INET_LED_BLINK) || defined(GPL_CODE_SMARTLED)
...
int hpna_logical_port = 0;
int hpna_dev_index = 0;
#endif
...
static void AEI_MirrorPacket( struct sk_buff *skb, char *intfName, UBOOL8 mirrorIn)
{
...
#endif
    {
        if( (skbCopy = skb_copy(skb, GFP_ATOMIC)) != NULL )
        {
            ...
            if (mirrorIn)
            {
                //because the eth mac will be checked in BCM53115. it is not used in BCM63100.
                //and will cause the checksum error when swap mac addr in BCM63100.
...
#if defined(BCM_ENET_CB_WAN_PORT_LNX_INTF_SUPPORT)
		{
		    ETHERNET_CROSSBAR_INFO *cb_info = NULL;

		    if (cb_port != BP_CROSSBAR_NOT_DEFINED)
			cb_info = &pVnetDev0->EnetInfo[unit].sw.crossbar[cb_port];

		    printk("Virtual Port: <%s> <vport_id %d> <unit %s> <physical %d> <logical %02d> <crossbar_port %d> <phy_id: 0x%08x> <MAC: %02X:%02X:%02X:%02X:%02X:%02X>\n",
			    dev->name, vport_id, (LOGICAL_PORT_TO_UNIT_NUMBER(logical_port)?"External":"Internal/Runner"), 
			    physical_port, logical_port, cb_port, 
			    (unsigned int)((cb_port != BP_CROSSBAR_NOT_DEFINED) ? cb_info->phy_id : phy_id), 
...
#if defined(GPL_CODE_HPNA)
        if (hpna_support && ((phy_id & BCM_PHY_ID_M) == HPNA_PHY_ID))
        {
            hpna_logical_port = logical_port;
            hpna_dev_index = vport_id;
        }
#endif
...
#if defined(CONFIG_BCM_GMAC)
        if ( IsGmacPort(logical_port) && IsWanPort(phy_id) )
        {
            gmac_set_wan_port( 1 );
        }
#endif
...
#if defined(GPL_CODE) && defined(BCM_ENET_CB_WAN_PORT_LNX_INTF_SUPPORT)
        {
           ETHERNET_CROSSBAR_INFO *cb_info = NULL;
...
#if defined(GPL_CODE_DUAL_RGMII)
    {
	int grp_num;
	for (grp_num = 0; grp_num < BCM_SW_MAX_TRUNK_GROUPS; grp_num++)
	{
            printk ("Trunk Group %d: master_port 0x%04lX, port_mask 0x%04lX\n", grp_num,
                ((trunk_grp[grp_num] & TRUNK_GRP_MASTER_PORT_MASK) >> TRUNK_GRP_MASTER_PORT_SHIFT), 
		trunk_grp[grp_num] & TRUNK_GRP_PORT_MASK);
	}
    }
#endif
...
/*In BCM 63138 B0, the LED of WAN Ethernet is driverd by HW, A0 is driverd by SW*/
#if defined(GPL_CODE_63138_CHIP) && defined(GPL_CODE)// && defined(CONFIG_BRCM_CHIP_VER_A0)

static void bcmenet_set_spdled_waneth(int speed)
{
    kerSysLedCtrl(kLedWanEthLink, speed ? kLedStateOn : kLedStateOff);

    if ( speed == 1000 )
    {
        kerSysLedCtrl(kLedWanEthSpeed1000, kLedStateOn);
    }
    else if (speed == 100 || speed == 200)
    {
        kerSysLedCtrl(kLedWanEthSpeed1000, kLedStateOff);
        kerSysLedCtrl(kLedWanEthSpeed100, kLedStateOn);
    }
    else if (speed == 10)
    {
        kerSysLedCtrl(kLedWanEthSpeed100, kLedStateOff);
        kerSysLedCtrl(kLedWanEthSpeed1000, kLedStateOff);
    }
    else
    {
...
void bcmenet_set_eth_sfp_led(int speed)
{
    if ( speed == 1000 )
    {
        kerSysLedCtrl(kLedWanSfpLink0, kLedStateOn);
        kerSysLedCtrl(kLedWanSfpLink1, kLedStateOn);
    }
    else if (speed == 100 || speed == 200)
    {
        kerSysLedCtrl(kLedWanSfpLink0, kLedStateOff);
        kerSysLedCtrl(kLedWanSfpLink1, kLedStateOn);
    }
    else if (speed == 10)
    {
        kerSysLedCtrl(kLedWanSfpLink0, kLedStateOn);
        kerSysLedCtrl(kLedWanSfpLink1, kLedStateOff);
    }
    else
    {
        kerSysLedCtrl(kLedWanSfpLink0, kLedStateOff);
...
void link_change_handler(int port, int cb_port, int linkstatus, int speed, int duplex)
{
    ...
    int linkMask;
    int sw_port;
#if defined(CONFIG_BCM_SWITCH_PORT_TRUNK_SUPPORT) && defined(GPL_CODE)
    ...
    if (pNetDev) 
    {
        ...
        {
#if defined(CONFIG_BCM_SWITCH_PORT_TRUNK_SUPPORT) && defined(GPL_CODE)
            int true_logical_port = port;
#endif
            vport = cbport_to_vportIdx[cb_port];
            pNetDev  = vnet_dev[vport];
            cb_priv = (BcmEnet_devctrl *)netdev_priv(pNetDev);
#if defined(GPL_CODE_MUTIL_WAN_ETHERNET_PHY)
            if ((g_wan_eth_link_up && !g_wan_sfp_link_up && pNetDev->name && !strcmp(pNetDev->name, "ewan1")) ||
                (g_wan_sfp_link_up && !g_wan_eth_link_up && pNetDev->name && !strcmp(pNetDev->name, "ewan0")))
            {
...
#endif
#if defined(GPL_CODE)
            cb_link_state_changed = (cb_priv->linkState != linkstatus);
#endif
            cb_priv->linkState = linkstatus; /* Store the link status in the network device itself */
#if defined(GPL_CODE)
            phyId = priv->EnetInfo[0].sw.crossbar[cb_port].phy_id;
#endif
#if defined(GPL_CODE_SUPPORT_SCKIPIO_GFAST)
            if(pNetDev->name && !strcmp(pNetDev->name,"ewan1"))
            {
                g_sfp_flag = linkstatus;
            }
#endif
	    ...
	    {
                true_logical_port = (trunk_grp[grp_no] & TRUNK_GRP_MASTER_PORT_MASK) >> TRUNK_GRP_MASTER_PORT_SHIFT;
	    }
	    if (cb_priv->sw_port_id != true_logical_port)
#else
...
#if defined(GPL_CODE)
    if (cb_link_state_changed || (priv->linkState & mask) != linkMask) {
#else
...
#if defined(CONFIG_BCM_JUMBO_FRAME) && !defined(CONFIG_BCM_MAX_MTU_SIZE)
    {
        int we_locked_rtnl = FALSE;
            ...
            if (LOGICAL_PORT_TO_UNIT_NUMBER(port) == 0)
            {
#if 1
                if (IsPhyAdvCapConfigValid(phyId))
                {
                    bcmenet_set_spdled_waneth(speed);
#if defined(GPL_CODE_MUTIL_WAN_ETHERNET_PHY)
                    g_wan_eth_link_up = TRUE;
#endif
                }
#endif
                if ((phyId & MAC_IFACE) == MAC_IF_SERDES)
                {
#if !defined(GPL_CODE_SFP_LED)
                    bcmenet_set_eth_sfp_led(speed);
#endif
                }
            }
#endif

#if defined(GPL_CODE_63138_CHIP) && defined(CONFIG_BRCM_CHIP_VER_A0)
            if (LOGICAL_PORT_TO_UNIT_NUMBER(port) == 0 && IsPhyAdvCapConfigValid(phyId))
               bcmenet_set_spdled_waneth(speed);
#endif
...
#if defined(GPL_CODE_SUPPORT_100M_ETH_RELINK)
            /* This is a workaround for 1/10 chance that 8-wire cable can link at 100M on.
             * the 63138 switch. This workaround uses a toggle "relink" parameter to reset the phy 
             * so non-1000M device relinks once when connected to WAN and LAN Ethernet ports. 
             */
            {
                uint16 phyport = LOGICAL_PORT_TO_PHYSICAL_PORT(port);
                int sw_unit = LOGICAL_PORT_TO_UNIT_NUMBER(port);
                if (is_cabled_eth_port(sw_unit,port))
                {
                    if (speed!=1000)
                    {
                        if (!relinked[port])
                        {
                            ...
                            ethsw_phyport_rreg(port, reg, &v16);
                            v16 |= BMCR_PDOWN;
                            ethsw_phyport_wreg(port, reg, &v16);
                ...
                printk((KERN_CRIT "%s (%s switch port: %d) (Logical Port: %d) Link UP %d mbps %s duplex\n"),
                    pNetDev->name, (sw_unit ?"Ext":"Int"),phyport,
            ...
            if (LOGICAL_PORT_TO_UNIT_NUMBER(port) == 0)
	        {
		       ...
		       if (IsPhyAdvCapConfigValid(phyId))
		       {
		           bcmenet_set_spdled_waneth(0);
#if defined(GPL_CODE_MUTIL_WAN_ETHERNET_PHY)
		           g_wan_eth_link_up = FALSE;
#endif
		       }
#endif
		        if ((phyId & MAC_IFACE) == MAC_IF_SERDES)
...
-------------------------------------------------------------------------- */
#if defined(GPL_CODE_LOAD_BALANCING_CONTROL)
static int start_load_balancing;

static void bcmenet_configure_load_balancing(int val)
{
    start_load_balancing = val;
    printk("%s load balancing on trunk ports\n", start_load_balancing ? "Started" : "Stopped");
}

#if defined(GPL_CODE_QTN_RGMII_RELOAD)
char mini_uboot_mac[6] = {0x0, 0x26, 0x86, 0x0, 0x0, 0x0};
#endif
#endif
...
#if defined(GPL_CODE_LOAD_BALANCING_CONTROL)
        /* This need to be put in a customer/feature specific define later on */
        if (start_load_balancing)
            port_to_use = hash_value%num_active_ports;
#if defined(GPL_CODE_QTN_RGMII_RELOAD)
        /* binding mini-uboot mac address with port-4 */
        if (0 == memcmp(pDa, mini_uboot_mac, 6))
            port_to_use = 0;
#endif
#else
        port_to_use = hash_value%num_active_ports;
...
void inline get_mark_pNbuff(pNBuff_t *pNBuff, uint32_t **mark)
{
    ...
    {
        *mark = &((struct sk_buff *)pBuf)->mark;
    }
    else
    {
        *mark = &((FkBuff_t *)pBuf)->mark; 
    }
}
...
#if defined(CONFIG_BCM96328) || defined(CONFIG_BCM96362) ||  defined(CONFIG_BCM963268) ||    defined(CONFIG_BCM96318) ||     defined(CONFIG_BCM63381) ||      defined(CONFIG_BCM963138) ||       defined(CONFIG_BCM963148)
...
static inline int bcm63xx_enet_xmit2(struct net_device *dev, EnetXmitParams *pParam)
{
    ...
    if (pEthCtrl->usMirrorOutFlags != 0)
    {
        ...
        skb = PNBUFF_2_SKBUFF(pParam->pNBuff);
...
#if !defined(CONFIG_BCM96838) && !defined(CONFIG_BCM963138) && !defined(CONFIG_BCM963148) && !defined(CONFIG_BCM96848)  /* FIXME: Temporary work around */
            ENET_TX_UNLOCK();
#endif
...
int enetLogPort2DevName(int portMap, char *buf, int bufLen)
{
    ...
    for (i = 0; i < MAX_TOTAL_SWITCH_PORTS; i++)
    {
...
#if defined(BCM_ENET_CB_WAN_PORT_LNX_INTF_SUPPORT)
        {
            ...
            {
#if defined(GPL_CODE)
                vport_idx = cbport_to_vportIdx[cb_port];
#else
...
int __init bcm63xx_enet_probe(void)
{
    ...
    if ( strstr(board_id, "C1000") != NULL || strstr(board_id, "C1900") != NULL )
        hpna_support = 0;
    else
        hpna_support = 1;
#endif

#if defined(GPL_CODE) && !defined(GPL_CODE_63168_CHIP) && !defined(GPL_CODE_63138_CHIP)
...
/* XXX: Move to system init / add ioctl */
    ...
    for (j = rdpa_if_lan0+G9991_MGMT_SID+1; j <= rdpa_if_lan0+G9991_MGMT_SID_MAX+1 ; j++)
    {
...
#if defined(GPL_CODE) && defined(CONFIG_BCM96368)
    {
        uint16 v16;
        extsw_rreg(PAGE_CONTROL, REG_WAN_PORT_MAP, (uint8 *)&v16, 2);
        v16 = swab16(v16);
        pVnetDev0->wanPort=v16;
    }
#endif
    ...
    switch (cmd)
    {
        ...
        case SIOCGSWITCHPORT:       /* Get Switch Port. */
            {
                int unit = 0, port = 0;
                val = -1;
                if (bcm63xx_enet_getPortFromName(enetif_data->ifname, &unit, &port))
                {
                    return -EFAULT;
                }
                val = PHYSICAL_PORT_TO_LOGICAL_PORT(port, unit);
...
#if defined(GPL_CODE_HPNA)
            if (hpna_support && (hpna_logical_port == port_id_from_dev(dev)))
            {
                val = AEI_ethsw_get_hpna_link_status();
            }
#endif
...
#endif
            if (val == 0) /* If no sub_port, return master port back */
            {
                val = 1 << LOGICAL_PORT_TO_PHYSICAL_PORT(swPort);
            }
        ...
        case SIOCSWANPORT:
            ...
            if ( (int)data ) {
                ...
                if ( IsGmacPort(swPort) )
                {
                    ...
                    gmac_set_wan_port( 1 );
                    ethsw_eee_port_enable(swPort, 1, priv->linkState & (1<<swPort));
                }
#endif
        ...
        case SIOCETHSWCTLOPS:
            switch(e->op) {
                ...
                case ETHSWQUEMAP:
                    return bcmeapi_ioctl_que_map(pDevCtrl, e);
                    break;

                case ETHSWQUEMON:
                    return bcmeapi_ioctl_que_mon(pDevCtrl, e);
                    break;
                ...
                case ETHSWRDPAPORTGET:
                    ...
                    return bcmeapi_ioctl_ethsw_phys_port_to_rdpa_port(e->unit, e->port, &e->val);
                    break;

                case ETHSWRDPAPORTGETFROMNAME:
                    BCM_ENET_DEBUG("ethswctl ETHSWRDPAPORTGETFROMNAME ioctl");
                    return bcmeapi_ioctl_ethsw_ifname_to_rdpaif(e);
                    break;
                ...
                case ETHSWPHYMODE:
                    ...
                    {
                        e->unit = LOGICAL_PORT_TO_UNIT_NUMBER(pDevCtrl->sw_port_id);
                        e->port = LOGICAL_PORT_TO_PHYSICAL_PORT(pDevCtrl->sw_port_id);
                    }
                    ...
                    else
                    {
                        ...
                        phy_id = pDevCtrl->EnetInfo[e->sub_unit].sw.crossbar[BP_PHY_PORT_TO_CROSSBAR_PORT(e->sub_port)].phy_id;
                    }
...
#endif /* CONFIG_BCM_SWITCH_PORT_TRUNK_SUPPORT */
#if defined(GPL_CODE_MUTIL_WAN_ETHERNET_PHY)
                case ETHSWCROSSBARCONFIG:
                    {
                        bcmeapi_ioctl_ethsw_crossbar_config(dev);
                    }
#endif
                ...
                case ETHSWPHYEEE:
                    ...
                    break;
#if defined(GPL_CODE_LOAD_BALANCING_CONTROL)
                case ETHSWCFGLOADBALANCING:
                {
                    bcmenet_configure_load_balancing(e->val);
                }
                break;
#endif
#endif
                ...
                case ETHSWPORTSTORMCTRL:
                    ...
                    if(bcm63xx_enet_isExtSwPresent() && e->unit == 1) {
                        return bcmeapi_ioctl_extsw_port_storm_ctrl(e);
                    } 
                    break;
#if defined(GPL_CODE_EXT_SWITCH_PORT_ISOLATE)
                case ETHSWPORTISOLATED:
                    if (e->type == TYPE_SET) {
                        conf_isolated_port(e->val);
                    } else {
                        e->val = is_enable_isolated_port();
                    }
                    break;
#endif
        ...
        case SIOCETHCTLOPS:
            switch(ethctl->op) {
                ...
                case ETHINITWAN:
                    {
                        ...
                        if (ifname && copy_to_user((void*)&ethctl->ifname, (void*)ifname, strlen(ifname)))
                            val = -EFAULT;
                ...
                case ETHWANCONVERTLAN:
                    {
                        if (dev == vnet_dev[0])
                            return -EFAULT;

                        if (!(dev->priv_flags & IFF_WANDEV)) {
                            printk("ETHWANCONVERTLAN: is not WAN DEV\n");
                            return -EINVAL;
                        }

                        if ( ethctl->val ) {
                                dev->priv_flags |= IFF_AEI_WAN_TO_LAN;
                        } else {
                              dev->priv_flags &= ~IFF_AEI_WAN_TO_LAN;
                        }
                    }
        ...
        case SIOCGHPNALINKSTATUS:
        {
            ...
            link_status = AEI_ethsw_get_hpna_link_status();

            if (copy_to_user((void *)data, (void *)&link_status, sizeof(unsigned int)))
            {
                return -EFAULT;
            }
            val = 0;
        ...
        case SIOCSHPNALINKSTATUS:
        {
            ...
            if (copy_from_user((void*)&link_status, data, sizeof(unsigned int)))
            {
                return -EFAULT;
            }
            else
...
#if defined(GPL_CODE_EXT_SWITCH_PORT_ISOLATE)
        /* If have ext switch port isolated, we need skip those ports */
        if (is_enable_isolated_port()) {
            uint32_t isolated_portmask = get_isolated_portmask();
            fwdPortMap &= ~isolated_portmask;
            portMapLoop &= ~isolated_portmask;
        }
#endif
