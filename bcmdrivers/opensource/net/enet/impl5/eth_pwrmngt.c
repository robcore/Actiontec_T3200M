...
#if defined(GPHY_EEE_1000BASE_T_DEF) || defined(CONFIG_BCM_EXT_SWITCH) || defined(CONFIG_BCM96318) || defined(CONFIG_BCM963138) || defined(CONFIG_BCM963148) || defined(CONFIG_BCM963381) || defined(CONFIG_BCM96838) || defined(CONFIG_BCM96848)
...
int ethsw_shutdown_unused_macs(BcmEnet_devctrl *pDevCtrl)
{
    ...
    for (i = 0; i < MAX_SWITCH_PORTS; i++) {
        ...
        if ((pDevCtrl->EnetInfo[STARFIGHTER2_UNIT].sw.port_map & (1U<<phys_port)) == 0) {
...
int ethsw_shutdown_unused_phys(void)
{
    ...
    //Power off unused ports in the Quad-GPHY
    portmap = pVnetDev0->EnetInfo[STARFIGHTER2_UNIT].sw.port_map;

    phy_ctrl = ETHSW_REG->qphy_ctrl;
    if ((portmap & QGPHY_PORT_MASK) == 0x0) {
        ...
        phy_ctrl |= ((portmap & QGPHY_PORT_MASK) ^ QGPHY_PORT_MASK) << ETHSW_QPHY_CTRL_EXT_PWR_DOWN_SHIFT;
    }
