...
void ethsw_set_wanoe_portmap(uint16 wan_port_map)
{
...
#ifdef GPL_CODE_RESET_SWITCH_SETWANPORT
    uint8_t v8;
#endif
    uint16 map;
...
#ifdef GPL_CODE_RESET_SWITCH_SETWANPORT
	/* reset switch when set wan port preventing abnormal status */
	/* 53125 connect to RGMII3 ,port is 6 */
    ethsw_rreg(PAGE_CONTROL, REG_PORT_CTRL + 6, &v8, 1);
    v8 &= (~REG_PORT_CTRL_DISABLE);
    v8 |= REG_PORT_CTRL_DISABLE;
    ethsw_wreg(PAGE_CONTROL, REG_PORT_CTRL + 6, &v8, 1);
#endif
    ...
    map |= GET_PORTMAP_FROM_LOGICAL_PORTMAP(pVnetDev0_g->learningDisabledPortMap, 1);
...
#if defined(GPL_CODE_EXTSW_DEBUG)
...
void bcmeapi_ethsw_dump_page(int page)
{
    switch (page) {
        ...
        case 0x27:
        case 0x28:
            bcmeapi_ethsw_dump_mib(page - 0x20, 1);
            break;
#if defined(GPL_CODE_EXTSW_DEBUG)
        case 0x1:
