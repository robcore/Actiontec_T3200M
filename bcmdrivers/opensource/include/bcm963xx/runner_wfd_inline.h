...
static inline void wfd_fwd_mcast(void *nkb_p, wfd_object_t *wfd_p, uint16_t ssid_vector, int is_fkb)
{
    ...
    while (ssid_vector)
    {
        ...
        (void) wfd_p->wfd_mcastHook((uint32_t)nkbC_p, wfd_p->wl_radio_idx, (uint32_t)wfd_p->wl_if_dev[wl_if_index]);
    }
   ...
   for (pktidx = 0; pktidx < rx_pktcnt; pktidx++)
   {
      ...
      if (!fkb_p->wl.ucast.dhd.is_ucast)
      {
         if (ucast_cnt)
         {
            ...
            (void) wfd_p->wfd_fwdHook(ucast_cnt, (uint32_t)ucast_pkts, wfd_p->wl_radio_idx, 0);
