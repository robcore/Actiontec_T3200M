...
typedef enum {
   ...
   RDPA_DECL(RDPA_IOC_RDPA_MW_SET_MCAST_DSCP_REMARK)
   RDPA_DECL(RDPA_IOC_RDPA_MW)
   RDPA_DECL(RDPA_IOC_TIME_SYNC)
   RDPA_DECL(RDPA_IOC_FILTERS)
   RDPA_DECL(RDPA_IOC_DSCP_TO_PBIT)
   RDPA_DECL(RDPA_IOC_PBIT_TO_Q)
   RDPA_DECL(RDPA_IOC_MISC)
...
typedef struct {
   ...
   uint32_t drop_alg;
   uint32_t red_high_thr;
   uint32_t red_low_thr;
   uint32_t cfg_flags;
