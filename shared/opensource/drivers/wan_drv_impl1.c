...
int init_wan_serdes(void)
{
    ...
    if (strcmp(wan_type_buf, "GPON") == 0)
    {
        wan_serdes_config(SERDES_WAN_TYPE_GPON);
    }
    else if (strcmp(wan_type_buf, "EPON") == 0)
    {
        ...
        if (!strcmp(epon_speed_buf, "Turbo"))
        {
            wan_serdes_config(SERDES_WAN_TYPE_EPON_2G);
        }
        else if (!strcmp(epon_speed_buf, "Normal"))
        {
            wan_serdes_config(SERDES_WAN_TYPE_EPON_1G);
        }
    }
    else if (strcmp(wan_type_buf, "GBE") == 0)
    {
...
int procfile_read(char *buffer, char **buf_location, off_t offset, int buf_len, int *eof, void *data)
{
    serdes_error_t rc;
    uint16_t ampl;
...
int procfile_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
    ...
    return (int)procfs_buffer_size;
}


int init_serdes_proc(void)
{
    serdes_proc_file = create_proc_entry(procfs_name, 0644, NULL);
    ...
    serdes_proc_file->read_proc = procfile_read;
    serdes_proc_file->write_proc = procfile_write;
