...
int runner_reserved_memory_get( uint32_t *tm_base_addr, uint32_t *tm_base_addr_phys,
    uint32_t *mc_base_addr, uint32_t *mc_base_addr_phys, uint32_t *tm_size)
{
    int rc;
    uint32_t mc_size;

    rc = BcmMemReserveGetByName(TM_BASE_ADDR_STR, tm_base_addr, tm_size);
    if (rc)
    {
    ...
    {
        rc = BcmMemReserveGetByName(TM_MC_BASE_ADDR_STR, mc_base_addr, &mc_size);
        if (rc)
        {
        ...
        {
            memset(*tm_base_addr, 0x00, *tm_size);
            memset(*mc_base_addr, 0x00, mc_size);

            *tm_base_addr_phys = (void*)virt_to_phys(*tm_base_addr);

            printk("tm_base_addr 0x%p, size %u, tm_base_addr_phys 0x%p\n",
                *tm_base_addr, *tm_size, *tm_base_addr_phys);

            *mc_base_addr_phys = (uint32_t)virt_to_phys(*mc_base_addr);

            printk("mc_base_addr 0x%p, size %u, mc_base_addr_phys 0x%08x\n",
                *mc_base_addr, mc_size, *mc_base_addr_phys);
...
int rdp_post_init(void)
{
    ...
    mac_hwapi_set_unimac_cfg(DRV_BBH_EMAC_0);
    ...
    mac_hwapi_set_unimac_cfg(DRV_BBH_EMAC_1);
