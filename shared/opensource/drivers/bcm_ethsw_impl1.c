...
void bcm_ethsw_init(void)
{
    ...
    for( i = 0; i < 5; i++ )
    {
        ...
        ETHSW_REG->led_ctrl[i] |=
            (ETHSW_LED_CTRL_SPD0_ON | ETHSW_LED_CTRL_SPD1_ON) << ETHSW_LED_CTRL_1000M_SHIFT |
            (ETHSW_LED_CTRL_SPD0_OFF | ETHSW_LED_CTRL_SPD1_ON) << ETHSW_LED_CTRL_100M_SHIFT |
            (ETHSW_LED_CTRL_SPD0_ON | ETHSW_LED_CTRL_SPD1_OFF) << ETHSW_LED_CTRL_10M_SHIFT |
            (ETHSW_LED_CTRL_SPD0_OFF | ETHSW_LED_CTRL_SPD1_OFF) << ETHSW_LED_CTRL_NOLINK_SHIFT;
    }
    ...
    if ( (CHIP_FAMILY_ID_HEX == 0x63148) || ((CHIP_FAMILY_ID_HEX == 0x63138) && (UtilGetChipRev() != 0xa0)) ) {
        ...
        ETHSW_REG->led_wan_ctrl |=
            ...
            (ETHSW_LED_CTRL_SPD0_ON | ETHSW_LED_CTRL_SPD1_OFF) << ETHSW_LED_CTRL_10M_SHIFT |
            (ETHSW_LED_CTRL_SPD0_OFF | ETHSW_LED_CTRL_SPD1_OFF) << ETHSW_LED_CTRL_NOLINK_SHIFT;
#define SPDLNK_SRC_SEL_BIT 0x01
#define SPDLINK_SRC_SEK_BIT_MASK 24
    ETHSW_REG->led_wan_ctrl |= (SPDLNK_SRC_SEL_BIT << SPDLINK_SRC_SEK_BIT_MASK);
    }
#else
#if defined(GPL_CODE)   /* Standard Actiontec behaviour */
    for( i = 0; i < 5; i++ )
    {
        ETHSW_REG->led_ctrl[i] &= ~ETHSW_LED_CTRL_SPEED_MASK;
        ETHSW_REG->led_ctrl[i] |=
	...
	/* 
	 * To blink back panel LEDs upon activity/traffic we need to write
	 * 1s in bits 8-15 of LED Port Control register
	 */
        #define ETHSW_LED_CTRL_BLINK0_ON    1
        #define ETHSW_LED_CTRL_BLINK0_OFF   0
        #define ETHSW_LED_CTRL_BLINK1_ON    2
        #define ETHSW_LED_CTRL_BLINK1_OFF   0
        #define ETHSW_LED_CTRL_BLINK_1000M_SHIFT    14
        #define ETHSW_LED_CTRL_BLINK_1000M_SHIFT    14
        #define ETHSW_LED_CTRL_BLINK_100M_SHIFT     12
        #define ETHSW_LED_CTRL_BLINK_10M_SHIFT      10
        #define ETHSW_LED_CTRL_BLINK_NOLINK_SHIFT    8
        ETHSW_REG->led_ctrl[i] |=
            (ETHSW_LED_CTRL_BLINK0_ON | ETHSW_LED_CTRL_BLINK1_ON) << ETHSW_LED_CTRL_BLINK_1000M_SHIFT |
            (ETHSW_LED_CTRL_BLINK0_OFF | ETHSW_LED_CTRL_BLINK1_ON) << ETHSW_LED_CTRL_BLINK_100M_SHIFT |
            (ETHSW_LED_CTRL_BLINK0_ON | ETHSW_LED_CTRL_BLINK1_OFF) << ETHSW_LED_CTRL_BLINK_10M_SHIFT |
            (ETHSW_LED_CTRL_BLINK0_OFF | ETHSW_LED_CTRL_BLINK1_OFF) << ETHSW_LED_CTRL_BLINK_NOLINK_SHIFT;
    }
#else  /* Original Broadcom behaviour */
