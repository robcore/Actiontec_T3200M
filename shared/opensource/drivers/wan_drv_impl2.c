...
static void onu2g_init(serdes_wan_type_t wan_type)
{
    ...
    // Step 2. We already have stable refclk from crystal
    udelay(50);
    
    // Step 3. De-assert pmd_por_h_rstb pin
    READ_32(WAN_MISC_WAN_TOP_WAN_MISC_ONU2G_MISC_CTRL ,onu2g_misc_ctrl);
    ...
    case SERDES_WAN_TYPE_EPON_1G:
        /* additional settings from Ryan */
#if 0
        onu2g_write(0x800D0B4, 0x0300); // Reduce IQ clock BW
        onu2g_write(0x800D093, 0x0030); // Increase PI current
        onu2g_write(0x800D091, 0x000C); // Increase PI current
#endif

        onu2g_write(0x800D004, 0x4020);
        onu2g_write(0x800D004, 0x4220);
        onu2g_read(0x800D009, &data);
        /* printk("+++ onu2g_read(0x800D009) == %x\n", data); */
        /* .. end */

        /* 2/4 test */
        onu2g_write(0x0800d0e4, 0x0001); /* loop timing */
        /* .. end */

    case SERDES_WAN_TYPE_EPON_2G:
        onu2g_write(0x0800d127, 0x7005); // 4. pll_mode=0x5 for 2.5G rate
        break;
    ...
    // skip 4B. (doubler) because we are using 50Mhz

    onu2g_write(0x0800d0b1, 0x8081); // 4B. (ams_pll_vco_div4_val)
    //Added by Ryan on 31/3
    onu2g_write(0x0800d0b4, 0x6377); // 4B. (ams_pll_force_kvh_bwh) + mario (ams_pll_kvh_force, bit 13) + | 0x300
    //Added by Ryan on 1/4
    onu2g_write(0x0800d094, 0xf000);
    onu2g_write(0x0800d093, 0x8);
    onu2g_write(0x0800d091, 0x2);
    onu2g_write(0x0800d0b3, 0x4);
    onu2g_write(0x800d0b2, 0x0196);
    onu2g_write(0x800d092, 0x2830);
    ...
    onu2g_write(0x0800d074, 0x0); // tx_pi_frz_mode = 0
    onu2g_write(0x0800d010, 0x208); // ignore sigdet

    onu2g_read(0x0800d040, &data);
    onu2g_write(0x0800d040, data | 0x0010);
    onu2g_write(0x0800d0f4, 0x2271); // (3A + 10.) + start PLL calibration
