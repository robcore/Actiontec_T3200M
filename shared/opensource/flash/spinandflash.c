...
#define SPI_NAND_DEVICES                                                  \
    {{SPI_MAKE_ID(GIGADEVPART, ID_GD5F1GQ4UA), "GigaDevice GD5F1GQ4UA"},  \
     ...
     {SPI_MAKE_ID(WINBONDPART, ID_W25N01GV),   "Winbond W25N01GV"},       \
     {0,""}                                                               \
    }
...
int spi_nand_read_page(unsigned long page_addr, unsigned int page_offset, unsigned char *buffer, int len, int count_bad_bits);
#else
...
static int spi_nand_read_cfg(PCFE_SPI_NAND_CHIP pchip)
{
    ...
    pchip->chip_ecc_enh = 0; // enhanced bad bit detection by chip
    ...
    case SPI_MAKE_ID(ESMTPART, ID_F50L1G41A): // 1Gb, 128MB
        ...
        pchip->chip_ecc_corr = 1; // bad bit threshold to mark page bad (1/1)
#endif
        break;

    case SPI_MAKE_ID(WINBONDPART, ID_W25N01GV): // 1Gb, 128MB, default part
#if defined(CFG_RAMAPP)
        pchip->chip_ecc_corr = 1; // bad bit threshold to mark page bad (1/1)
#endif
        break;
...
#endif
{
    PCFE_SPI_NAND_CHIP pchip = &g_spinand_chip;
#if defined(CFG_RAMAPP)
    unsigned char buf[(READ_BUF_LEN_MAX > pchip->chip_spare_size) ? READ_BUF_LEN_MAX : pchip->chip_spare_size];
#else
...
{ // can only read one block at a time, otherwise if one block is good and another is bad then whole read is invalid
    ...
    {
#if defined(CFG_RAMAPP)
        if (spi_nand_read_page(page_addr, page_offset, buffer, size, 0) == FLASH_API_ERROR)
#else
...
static int spi_nand_write_page(unsigned long page_addr, unsigned int page_offset, unsigned char *buffer, int len)
{
    ...
    if (verify)
    {
        ...
        status = spi_nand_read_page(page_addr, 0, buf, pchip->chip_page_size, 1);
    ...
    if (len)
    {
        ...
        do
        {
            ...
            { // read/erase/write block to see if we can get rid of the bit errors, but only if not block zero
                ...
                {
                    status = spi_nand_read_page(block_addr + offset, 0, buffer + offset, pchip->chip_page_size, 0);
                    if (status == FLASH_API_ERROR)
                        error = 1;
                }
...
static int spi_nand_is_blk_bad(PCFE_SPI_NAND_CHIP pchip, unsigned short blk)
{
    ...
    spi_nand_read_page(pchip->chip_block_size * blk, pchip->chip_page_size, &buf1, 1, 0);
    spi_nand_read_page((pchip->chip_block_size * blk) + pchip->chip_page_size, pchip->chip_page_size, &buf2, 1, 0);
    g_no_ecc = temp;
...
int spi_nand_is_cleanmarker(unsigned long addr, int write_if_not)
{
    ...
    if (spi_nand_read_page(page_addr, pchip->chip_page_size, buf, pchip->chip_spare_size, 0) == FLASH_API_ERROR)
    {
        return(FLASH_API_ERROR);
    }
...
int read_spi_spare_data(int blk, int offset, unsigned char *buf, int bufsize)
{
    ...
    return(spi_nand_read_page(page_addr, pchip->chip_page_size, buf, bufsize, 0));
}
