...
#define BCM63138_TM_DEF_DDR_SIZE                    0x1400000
...
/* high congestion threshold of runner A (DS), for DSL mode */
#define IH_DSL_MODE_DS_RUNNER_A_HIGH_CONGESTION_THRESHOLD    0x1E
/* exclusive congestion threshold of runner A (DS), for DSL mode */
#define IH_DSL_MODE_DS_RUNNER_A_EXCLUSIVE_CONGESTION_THRESHOLD 0x1E
...
#define IH_INGRESS_QUEUE_CONGESTION_THRESHOLD_LAN_EMACS 31
/* congestion threshold of ingress queue of the WAN port */
#define IH_INGRESS_QUEUE_CONGESTION_THRESHOLD_WAN 65
#define IH_INGRESS_QUEUE_CONGESTION_THRESHOLD_WAN_DSL 0x1D /* 1 less than Max-0x1E */
...
static const uint32_t  bbh_to_rdd_eth_thread[] =
{
    ...
    ETH_TX_THREAD_NUMBER, /* interlan task for SF2 */
...
static void rdp_bpm_threshod_get(const uint32_t max_bpm_bufs, uint32_t *p_bpm_gbl_thresh, uint32_t *p_bpm_def_thresh)
{
    ...
    {
    #if 0 /* defined(DSL_63138)  - more than 15K buffers do not work for Dongle offload */
...
void rdp_bpm_cfg_params_get(const uint32_t runner_tm_size, rdp_bpm_cfg_params *p_bpm_cfg_params)
{
    uint32_t max_bpm_bufs, bpm_buf_size;

    bpm_buf_size = rdp_bpm_buf_size();
    
    max_bpm_bufs = (runner_tm_size*1024*1024 - RDP_DDR_DATA_STRUCTURES_SIZE)/bpm_buf_size;
    rdp_bpm_threshod_get(max_bpm_bufs, &p_bpm_cfg_params->bpm_gbl_thresh, &p_bpm_cfg_params->bpm_def_thresh);
    __print("\n RDP TM memory = %uMB : Max Possible Bufs <%u> of size <%u>; Allocating <%u> bufs; RDP enum <%u>\n",
           runner_tm_size, max_bpm_bufs, bpm_buf_size, p_bpm_cfg_params->bpm_def_thresh, p_bpm_cfg_params->bpm_gbl_thresh );
}
...
static void f_ih_init(void)
{
    ...
    runners_load_thresholds.runner_a_high_congestion_threshold      = IH_DSL_MODE_DS_RUNNER_A_HIGH_CONGESTION_THRESHOLD;
    runners_load_thresholds.runner_b_high_congestion_threshold      = DRV_RDD_IH_RUNNER_0_MAXIMAL_NUMBER_OF_BUFFERS;
    runners_load_thresholds.runner_a_exclusive_congestion_threshold = IH_DSL_MODE_DS_RUNNER_A_EXCLUSIVE_CONGESTION_THRESHOLD;
...
static void f_configure_runner(S_DPI_CFG  *pCfg, rdp_bpm_cfg_params *p_bpm_cfg_params)
{
    ...
    rdd_init_params.ddr_runner_tables_ptr = (uint8_t *) (pCfg->runner_tm_base_addr +
        (((p_bpm_cfg_params->bpm_def_thresh * rdp_bpm_buf_size()) + 0x7ffff) & ~0x7ffff));
...
uint32_t data_path_init( S_DPI_CFG  *pCfg)
{
    ...
    rdp_bpm_cfg_params_get(pCfg->runner_tm_size, &bpm_cfg_params);
...
void f_initialize_bbh_of_dsl_port(void)
{
    ...
    per_flow_configuration.default_ih_class = 8;
    per_flow_configuration.ih_class_override = 0;
    ...
    per_flow_configuration.minimum_packet_size_selection = BBH_RX_ETH_MIN_PKT_SIZE_SELECTION_INDEX;
    per_flow_configuration.maximum_packet_size_selection = BBH_RX_MAX_PKT_SIZE_SELECTION_INDEX;
    per_flow_configuration.default_ih_class = 8; // DRV_RDD_IH_CLASS_WAN_BRIDGED_LOW_INDEX;
    per_flow_configuration.ih_class_override = 1;
