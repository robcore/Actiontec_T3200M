...
#define CHIP_FAMILY_ID_HEX	0x6362
...
#define OTP_REGS_GET_USER_BIT(x)      ((OTP_REGS->UserBits[((sizeof(OTP_REGS->UserBits)/4) - (x)/32 - 1)] >> ((x) % 32)) & 1)
...
typedef struct DECTShimControlRegs_S 
{
...
} DECTShimControlRegs_S;

#define DECT_CTRL ((volatile DECTShimControlRegs_S * const) DECT_SHIM_CTRL_BASE)
