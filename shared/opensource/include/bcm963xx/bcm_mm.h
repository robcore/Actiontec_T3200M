...
#define CACHE_TO_NONCACHE(x)    			cache_to_uncache(((uint32_t)(x)))
#define NONCACHE_TO_CACHE(x)    			uncache_to_cache(((uint32_t)(x)))
#define CACHED_MALLOC(_size) 				uncache_to_cache((uint32_t)KMALLOC((_size),0))
#define NONCACHED_MALLOC(_size) 			cache_to_uncache((uint32_t)KMALLOC((_size),0))
#define NONCACHED_FREE(_ptr) 				KFREE(uncache_to_cache(_ptr))
#define CACHED_FREE(_ptr)				KFREE(_ptr)
#define VIRT_TO_PHYS(_addr)				uncache_to_cache((uint32_t)_addr)
#define PHYS_TO_CACHED(_addr)				((uint32_t)_addr)
#define PHYS_TO_UNCACHED(_addr)				cache_to_uncache((uint32_t)_addr)
...
#define INV_RANGE(_addr, _size) 			blast_inv_dcache_range(_addr, _addr+_size)
...
#define NONCACHED_MALLOC(_size, _phys_addr_ptr)		({void *_acp_addr_ptr = NULL;			\
		void *_org_addr_ptr = dma_alloc_coherent(NULL, _size + 4, _phys_addr_ptr, GFP_KERNEL|GFP_ACP);	\
		if (_org_addr_ptr != NULL) {								\
			_acp_addr_ptr = (void *)ACP_ADDRESS(*_phys_addr_ptr);				\
			*(uint32_t *)((uint32_t)_acp_addr_ptr + _size) = (uint32_t)_org_addr_ptr;	\
		}											\
		_acp_addr_ptr;		})
#define NONCACHED_FREE(_size, _ptr, _phys_addr)		({void *_org_addr_ptr;				\
			_org_addr_ptr = (void *)(*(uint32_t *)((uint32_t)_ptr + _size));	\
