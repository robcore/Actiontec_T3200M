...
#if defined(GPL_CODE_EBT_VLAN_EXT)
#include "../include/linux/netfilter_bridge/ebt_vlan.h"
#else
...
#if defined(GPL_CODE_EBT_VLAN_EXT)
	{"vlan-packet-content", required_argument, NULL, VLAN_PACKET_CONTENT},
#endif
	{ 0 }
};
...
#define OPT_VLAN_PACKET_CONTENT  0x08
#define OPT_VLAN_FLAGS	(OPT_VLAN_ID | OPT_VLAN_PRIO | OPT_VLAN_ENCAP | OPT_VLAN_PACKET_CONTENT)
   ...
   unsigned int *flags, struct ebt_entry_match **match)
{
	...
	case VLAN_ENCAP:
		...
		break;
#if defined(GPL_CODE_EBT_VLAN_EXT)
	case VLAN_PACKET_CONTENT:
	    if (!(vlaninfo->bitmask & EBT_VLAN_ENCAP)) 
		    ebt_print_error("For vlan packet content,  the vlan enacap must be specified");
		ethent = getethertypebynumber(ntohs(vlaninfo->encap));
		if ( ethent == NULL || strcasecmp(ethent->e_name, "IPv4") )
		    ebt_print_error("For vlan packet content,  the vlan enacap must be specified as IPv4");
		ebt_check_option2(flags, OPT_VLAN_PACKET_CONTENT);
		if (ebt_check_inverse2(optarg))
			vlaninfo->invflags |= EBT_VLAN_PACKET_CONTENT;
		if ( strcasecmp(optarg, "DHCP") )
			ebt_print_error2("Invalid --vlan-packet type ('%s')", optarg);
		strncpy(vlaninfo->pktcontent, optarg,sizeof(vlaninfo->pktcontent));
		vlaninfo->bitmask |= EBT_VLAN_PACKET_CONTENT;
		break;
#endif
	default:
		return 0;
...
#if defined(GPL_CODE_EBT_VLAN_EXT)
	if (vlaninfo->bitmask & EBT_VLAN_PACKET_CONTENT) {
		printf("--vlan-packet-content %s%s ", (vlaninfo->invflags & EBT_VLAN_PACKET_CONTENT) ? "! " : "", vlaninfo->pktcontent);
	}
#endif
}
   ...
   const struct ebt_entry_match *vlan2)
{
	...
	if (vlaninfo1->bitmask & EBT_VLAN_ENCAP &&
		...
		return 0;
#if defined(GPL_CODE_EBT_VLAN_EXT)
        if (vlaninfo1->bitmask & EBT_VLAN_PACKET_CONTENT  &&
            strncmp( vlaninfo1->pktcontent, vlaninfo2->pktcontent,sizeof(vlaninfo1->pktcontent)))
		return 0;
#endif
	return 1;
}
