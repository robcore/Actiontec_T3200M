...
int br_mark_dedicated_stp_port(const char *bridge, const char *ifName, int setting)
{
	int            err;
	struct ifreq   ifr;
	int            ifIndex = if_nametoindex(ifName);
	unsigned long  args[3] = { BRCTL_MARK_DEDICATED_STP, ifIndex, setting};
...
int br_block_stp(const char *bridge, const char *ifName)
{
	int            err;
	struct ifreq   ifr;
	int            ifIndex = if_nametoindex(ifName);
	unsigned long  args[2] = { BRCTL_BLOCK_STP, ifIndex };
