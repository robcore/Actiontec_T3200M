...
#if defined(GPL_CODE_DHCP_AUTO_RESERVATION) || defined(GPL_CODE)
#include "cms_msg.h"
#endif
...
int lease_time_remaining(const struct dhcpOfferedAddr *lease)
{
   ...
   if (lease->yiaddr)
   {
      ...
      else
      {
          ...
          end_time = time(0);
          val = end_time - select_begin_time + reduce_lease_time;

          if (lease->expires <= val)
              remain = 0;
          else
              remain = lease->expires - val;
      }
   }
...
{
	u_int32_t addr, ret = 0; /* addr is in network order */
	...
	for (;ntohl(addr) <= ntohl(cur_iface->end);
		...
		if(!reservedIp(cur_iface->static_leases, addr))
		{
			/* lease is not taken */
			ret = addr;
...
static int AEI_issue_ebtable(struct dhcpMessage *oldpacket)
{
        ...
        snprintf(ebtable_cmd, sizeof(ebtable_cmd), "/bin/ebtables -D STBCHAIN -s %02X:%02X:%02X:%02X:%02X:%02X -j ACCEPT > /dev/null",
                                    oldpacket->chaddr[0],
                                    oldpacket->chaddr[1],
                                    oldpacket->chaddr[2],
                                    oldpacket->chaddr[3],
         ...
         */
         ...
         snprintf(ebtable_cmd, sizeof(ebtable_cmd),"/bin/ebtables -A STBCHAIN -s %02X:%02X:%02X:%02X:%02X:%02X -j ACCEPT > /dev/null",
                                    oldpacket->chaddr[0],
                                    oldpacket->chaddr[1],
                                    oldpacket->chaddr[2],
                                    oldpacket->chaddr[3],
...
static int AEI_issue_ebtableVlan(struct dhcpMessage *oldpacket, char* vlanID)
{
        ...
        snprintf(ebtable_cmd, sizeof(ebtable_cmd), "/bin/ebtables -D STBCHAIN%s -s %02X:%02X:%02X:%02X:%02X:%02X -j ACCEPT > /dev/null",
                                    vlanID,
                                    oldpacket->chaddr[0],
                                    oldpacket->chaddr[1],
                                    oldpacket->chaddr[2],
        ...
        snprintf(ebtable_cmd, sizeof(ebtable_cmd), "/bin/ebtables -A STBCHAIN%s -s %02X:%02X:%02X:%02X:%02X:%02X -j ACCEPT > /dev/null",
                                    vlanID,
                                    oldpacket->chaddr[0],
                                    oldpacket->chaddr[1],
                                    oldpacket->chaddr[2],
        ...
        /* Add a rule to block ARP respond to the STB */
         snprintf(ebtable_cmd, sizeof(ebtable_cmd), "/bin/ebtables -D STBARPCHAIN%s -s %02X:%02X:%02X:%02X:%02X:%02X -j DROP > /dev/null",
                                    vlanID,
                                    oldpacket->chaddr[0],
                                    oldpacket->chaddr[1],
                                    oldpacket->chaddr[2],
         ...
         snprintf(ebtable_cmd, sizeof(ebtable_cmd), "/bin/ebtables -A STBARPCHAIN%s -s %02X:%02X:%02X:%02X:%02X:%02X -j DROP > /dev/null",
                                    vlanID,
                                    oldpacket->chaddr[0],
                                    oldpacket->chaddr[1],
                                    oldpacket->chaddr[2],
...
int AEI_is_vlan_vendor_equipped(struct dhcpMessage *oldpacket, char* buf)
{
	...
	p = (char *)get_option(oldpacket, DHCP_VENDOR);
	if (p)
	{
...
#ifdef GPL_CODE_DHCP_LEASE
void adjust_lease_time(long delta __attribute__((unused)))
#else
