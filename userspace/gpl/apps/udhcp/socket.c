...
int raw_socket(int ifindex)
{
	...
	/* setting a larger socket buffer size to avoid possible DHCP session failure
	 * due to heavy incoming traffic at the same time */
	n = 2048 * 1024;
	setsockopt(fd, SOL_SOCKET, SO_RCVBUFFORCE, (char *)&n, sizeof(n));
