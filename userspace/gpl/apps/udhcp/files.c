...
#if defined(GPL_CODE)
void AEI_writeDhcpLog(char *token, struct dhcpMessage *packet)
{
    char dInfo[256];

    sprintf(dInfo, "receive dhcp %s from %02x:%02x:%02x:%02x:%02x:%02x", token,
                                      packet->chaddr[0], packet->chaddr[1], packet->chaddr[2],
                                      packet->chaddr[3], packet->chaddr[4], packet->chaddr[5]);
    AEI_log_local_info("dhcpd", dInfo);
}
#endif
...
#if defined(GPL_CODE) //add william 2012-4-25
       struct dhcpvlanOption60 * vlancurr = NULL;
       struct dhcpvlanOption60 * vlanprev = NULL;
       vlancurr = iface->dhcpvlanOption60list;
...
#endif
#if defined(GPL_CODE_DHCP_OPTION60)
     for (i = 0; i < VENDOR_CLASS_ID_TAB_SIZE; i++) {
        if (iface->option60Vids[i])
            free(iface->option60Vids[i]);
    }
#endif
...
#endif
#if defined(GPL_CODE)
     struct ip_list *p_dns_srv_list = NULL;
     struct ip_list *n_dns_srv_list = NULL;
    
     for (p_dns_srv_list = iface->dns_srv_ips; p_dns_srv_list  != NULL; p_dns_srv_list = n_dns_srv_list) {
           n_dns_srv_list = p_dns_srv_list->next;
           free(p_dns_srv_list);
           p_dns_srv_list = NULL;
     }
...
#if defined(GPL_CODE)  //hk_qwest//hk_ctl
...
#ifdef GPL_CODE
static void read_vids(char *line, char **vids, int size)
{
    int i;
    char *token, *str;
...
static void AEI_read_dhcpdOption125(char *line)
{
    char *token = NULL;
    ...
    token = strtok(line, ",");
#if defined(GPL_CODE_COVERITY_FIX)
    strlcpy(deviceOui, token, VENDOR_GATEWAY_OUI_MAX_LEN + 1);
#else
    strlcpy(deviceOui, token, VENDOR_GATEWAY_OUI_MAX_LEN);
#endif

    token = strtok(NULL, ",");
    strlcpy(deviceSerialNum,token,VENDOR_GATEWAY_SERIAL_NUMBER_MAX_LEN);

    token = strtok(NULL, ",");
    strlcpy(deviceProductClass,token,VENDOR_GATEWAY_PRODUCT_CLASS_MAX_LEN);

#if defined(GPL_CODE_FEATURE_GATEWAY_CAPABILITY)
    token = strtok(NULL, ",");
    deviceCapability = (unsigned int)strtoul(token, NULL, 16);
#endif
...
#endif
/*Add by sunny for option60 2014-06-11*/
#if defined(GPL_CODE_DHCP_OPTION60)
                else if (strcasecmp(token, "option60_vid") == 0)
                        read_vids(line, cur_iface->option60Vids, VENDOR_CLASS_ID_TAB_SIZE);
#endif
/**********End*****************/
#if defined(GPL_CODE_DHCP_LEASE)
		...
		else if (strcasecmp(token, "spec_char") == 0)
        {
			read_u32(line, &server_config.specChar_enable);
        }
#endif
...
#endif
#if defined(GPL_CODE)
        else if (strcasecmp(token, "opt125_vendor_id") == 0) {
            AEI_read_dhcpdOption125(line);
        }
#endif
		else
...
#if 0 
/*not use*/
static long AEI_getUpTime_dhcpd(void)
{
    FILE *fp=NULL;
    float time1, time2;
			...
			if (iface->leases[i].yiaddr != 0) {
				...
				fwrite(iface->leases[i].hostname,
				       ...
				       1, fp);
                                ...
                                fwrite(iface->leases[i].WPCap, sizeof(iface->leases[i].WPCap), 1, fp);
#endif
#endif
#if defined(GPL_CODE)
...
// BRCM_begin
void send_release_lease(const struct saved_lease *lease)
{
   ...
   DEBUG(LOG_INFO, "sending release lease info update msg");
   ...
   {
      LOG(LOG_WARNING, "could not send release lease info update");
   }
   else
   {
      DEBUG(LOG_INFO, "release lease info update sent!");
   }
}
// BRCM_end
...
void read_leases(char *file)
{
	...
	/* Init cnt_leases value per interface */
	for (iface = iface_config; iface; iface = iface->next) {
		iface->cnt_leases=0;
	}
	while ((fread(&lease, sizeof lease, 1, fp) == 1)) {
		for (iface = iface_config; iface; iface = iface->next) {
			if (ntohl(lease.yiaddr) >= ntohl(iface->start) &&
				ntohl(lease.yiaddr) <= ntohl(iface->end) &&
...
#endif
#if defined(GPL_CODE_WP)
                    iface->leases[cur_iface->cnt_leases].isWP= lease.isWP;
#endif
                }
#endif
...
#if defined(GPL_CODE_WP_AUTO_CONFIG)
                memcpy(iface->leases[cur_iface->cnt_leases].WPCap, lease.WPCap, sizeof(iface->leases[cur_iface->cnt_leases].WPCap));
#endif
#endif
				iface->leases[iface->cnt_leases].yiaddr =
					lease.yiaddr;
				iface->leases[iface->cnt_leases].expires =
					ntohl(lease.expires);	
				if (server_config.remaining)
					iface->leases[iface->cnt_leases].
					expires += curr;
				memcpy(iface->leases[iface->cnt_leases].
					chaddr, lease.chaddr,
					sizeof(lease.chaddr));
				memcpy(iface->leases[iface->cnt_leases].
				...
				break;
// BRCM_begin
			}else{  /* Need to inform the ssk to clean the old leased entries, 
                                 * where the subnet doesn't match. */
				DEBUG(LOG_INFO, "release old lease entries. (subnet mis-matched!)");
				send_release_lease( &lease);
			}
// BRCM_end
		}
	}
