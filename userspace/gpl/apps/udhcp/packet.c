...
int get_packet(struct dhcpMessage *packet, int fd)
{
	...
	if (packet->op == BOOTREQUEST && (vendor = (char *)get_option(packet, DHCP_VENDOR))) {
...
#ifdef GPL_CODE
	payload_length = sizeof(struct dhcpMessage) - UDHCP_OPTIONS_LEN;
#else
...
#if defined(GPL_CODE)
        /*WindStream Request WAN keep up, SET DHCPC Traffic Highest priority */
        if (dest_port == SERVER_PORT) {
            	unsigned int mark = 0x7;
        	if (setsockopt(fd, SOL_SOCKET, SO_MARK, (char *)&mark, sizeof(mark)) < 0) {
        	 	DEBUG(LOG_ERR, "set QoS Priority failed:\n");
		}
        }
#endif
