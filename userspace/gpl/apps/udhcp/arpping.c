...
int arpCheck(u_long inaddr, struct ifinfo *ifbuf, long timeout, u_int8_t *chaddr,int isCopyMac, char*server_addr);
#else
int arpCheck(u_long inaddr, struct ifinfo *ifbuf, long timeout, u_int8_t *chaddr);
#endif
 ...
 *		-1 error 
 */  
int arpping(u_int32_t yiaddr, u_int32_t ip, char *interface, u_int8_t *chaddr) {
	struct ifinfo ifbuf;
	int fd, ret;
	struct ifreq ifr;
...
#if defined(GPL_CODE_ADVANCED_DMZ)
    return arpCheck(yiaddr, &ifbuf, 2,chaddr,0,NULL);
#else
    return arpCheck(yiaddr, &ifbuf, 2,chaddr);
#endif
}
#if defined(GPL_CODE_ADVANCED_DMZ)
/* ken, get the Server MAC address*/
int arppingForDHCPc(u_int32_t yiaddr, u_int32_t ip, unsigned char *arp, char *interface,int isCopyMac, char*server_addr)
{
    struct ifinfo ifbuf;
    ...
    return arpCheck(yiaddr, &ifbuf, 2, NULL,isCopyMac,server_addr);
}
#endif
		...
		if ( select(s+1, &fdset, (fd_set *)NULL, (fd_set *)NULL, &tm) < 0 ) {
			...
			if(arp.operation == htons(ARPOP_REPLY) && 
			   ...
			   *((u_int *)arp.sInaddr) == inaddr ) {
                ...
                else if(chaddr && memcmp(arp.sHaddr, chaddr, 6))
                {
                       rv =0;
                }
#else         
                 if(chaddr && memcmp(arp.sHaddr, chaddr, 6))
                  {
		      rv = 0;
		  }
#endif
