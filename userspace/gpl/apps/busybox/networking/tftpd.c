...
#if defined (GPL_CODE)
#define SUPPORT_IPV6
#if defined (AEI_QTN_WIRELESS_RGMII)
#define TFTPD_FW "/lib/firmware" /* quantenna firmware dir */
#define TFTPD_ROOT "/var/tmp"    /*tftpd dir */
#define QTN_STATUS_FILE "/var/has_qtn"    /* used to indicate QTN card */
#else
...
#if defined(GPL_CODE_QTN_WIRELESS_RGMII)
void aei_clear_request_queue(void)
{
    int fd = CMS_DYNAMIC_LAUNCH_SERVER_FD;
    char buf[TFTP_BLOCKSIZE_DEFAULT + 4];
    int count = 0;

    while (1) {
        if (0 >= recv(fd, buf, sizeof(buf), MSG_DONTWAIT)) {
            cmsLog_debug("clear the tftp request queue: count=%d\n", count);
            break;
        }
        count++;
    }
...
#if defined(GPL_CODE_QTN_WIRELESS_RGMII)
         /*
          * we notice that QTN will send tftp request (topaz-linux.lzma.img) to BCM Host
          * continiously with different source ports when one tftp request fails.
          * Since of single process TFTPD processing mechanism, if there are multiple requests,
          * the requests will be cached by kernel. If the speed of QTN TFTP down load request is
          * greater than the TFTPD's processing speed, it will cause the QTN loading failure
          * or loading time delay. So if TFTP process time out, we need clear the request of cache,
          * then next time, tftp will handle the last request.
          */
         aei_clear_request_queue();
#endif
         common_exit(0);
       }
     break;
...
#if defined(GPL_CODE_QTN_WIRELESS_RGMII)
/*update QTN side download FW times*/
static void aei_updateQtnLoadStatus(void)
{
    FILE *fp = NULL;
    fp=fopen(QTN_STATUS_FILE, "a");
    if(fp)
    {
       fputs("1\n", fp);
       fclose(fp);
...
#if !defined(GPL_CODE_TR181_WLMNGR)
#define QTN_INIT    1
#define QTN_RELOAD  2

/*chek current downloading is reloading*/
static int aei_checkQtnIsRelod(void)
{
    FILE *fp = NULL;
    char buf[128] = {0};
    int ret = 0;
    fp = fopen(QTN_STATUS_FILE, "r");
    if (fp != NULL)
    {
...
/*send message to WLMNGR to reconfig QTN wireless card*/
static void AEI_qtnRgmiiLoadMsg(void)
{
    char cmd[128]={0};
    char msgBuf[sizeof(CmsMsgHeader) + 100] = {0};
    CmsMsgHeader *msg;
    CmsRet ret = CMSRET_SUCCESS;
#if !defined(GPL_CODE_TR181_WLMNGR)
    int loadType = aei_checkQtnIsRelod();

    /* notify wlmngr to reload qtn and re-apply 5g-wifi setting*/
    if(loadType == QTN_INIT)
        strcpy(cmd, "init_wifi5G");
    else if(loadType == QTN_RELOAD)
        strcpy(cmd, "reset_wifi5G");
#else
    strcpy(cmd, "reset_wifi5G");
#endif
    msg = (CmsMsgHeader *) msgBuf;
    strcpy((char *) (msg + 1), cmd);
    msg->dataLength = strlen(cmd);
    msg->src = EID_TFTPD;
#if defined(GPL_CODE_TR181_WLMNGR)
    msg->dst = EID_SSK;
    msg->type = CMS_MSG_QTN_RGMII_RELOAD;
#else
    msg->dst = EID_WLMNGR;
    ...
    {
        cmsLog_error("Failed to send msg to wlmngr, ret=%d", ret);
    }
    else
    {
        cmsLog_error("Succeed to send  msg to wlmngr");
    }
}
#endif
#endif
...
tftpd_send (int peer, struct tftphdr *first_pkt, int pkt_len, int buffersize)
{
...
#if defined(GPL_CODE_QTN_WIRELESS_RGMII)
   if(!strcmp(list[0], "u-boot.bin") || !strcmp(list[0], "topaz-linux.lzma.img"))
   {
#if defined(GPL_CODE_QTN_RGMII_RELOAD)
       if(!strcmp(list[0], "u-boot.bin")) {
           system("ethswctl -c load_balancing -o disable");
       }
#endif
       snprintf(path, TFTPD_PATH_LEN, "%s/%s", TFTPD_FW, list[0]);
   }
   else
#endif
   ...
   fclose(file);
#if defined(GPL_CODE_QTN_RGMII_RELOAD)
   if(!strcmp(list[0], "topaz-linux.lzma.img"))
   {
       /* For QTN acR4.X, we found there would be 2 tftp requests for topaz-linux.lzma.img
        * only update Qtn load status after FW is loaded completely*/
       aei_updateQtnLoadStatus();
       AEI_qtnRgmiiLoadMsg();
   }
#endif
   common_exit(0);
