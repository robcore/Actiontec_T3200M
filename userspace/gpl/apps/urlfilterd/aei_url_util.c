...
#if defined(GPL_CODE_WLAN_URL_REDIRECT)
	...
	while(p != NULL)
	{
		...
		if ((pTemp = strchr(allowIpAddr, '/')) != NULL)
		{
			*pTemp = ' ';
            ...
            //Covert network byte order IP Address to host byte order, and then check with mask
			ipNet = ntohl(ip);
			ipAddr = ntohl(allowIP.s_addr);
			mask = mask<<(32-maskNum);
			if ((__be32)(ipAddr&mask) == (__be32)(ipNet&mask))
			{
				printf("find mask\n");
				return 1;
			}
...
/*
 * Function: AEI_getWizardFlag()
 *
 * Description: This function check the wizardPortalflag is set or not
 *
 ...
 *
 * Usage:
 * The function_name() function is used to for Frontier Wizard portal function
 *
 */
int AEI_getWizardFlag()
{
    if(access("/var/wizard_portal",R_OK)==0)
        return 1;
    else
        return 0;
}
