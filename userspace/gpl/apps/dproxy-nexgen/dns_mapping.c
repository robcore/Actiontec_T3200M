...
#if defined(GPL_CODE)
   srcPort = ntohs(((struct sockaddr_in *)lanInfo)->sin_port);
#else
...
#if defined(GPL_CODE)
/*
 * QA-Bug #221433 DUT reboots multiple times during overnight soak due to out of memory condition
 * There is one case:
 * When WAN side DHCP server only provide one DNS server to our Gateway, we have only one available here,
 * which should be ipv4_dns1, and the other one should be "0.0.0.0"
 * In this case, if DNS query doesn't work on the primary DNS ipv4_dns1, and when we swap,
 * dnsproxy will use "0.0.0.0" as WAN DSN server
 *
 * In this case, on one hand dnsproxy use "0.0.0.0" as destination DNS IP, bind, send DNS query to WAN side,
 * on the other hand dnsproxy monitor DNS query from LAN side, the previous pkt sento "0.0.0.0" will be treated as a LAN side packet from "127.0.0.1"
 * it will immediately send this packet to WAN side DSN Server "0.0.0.0",
 * It result in a DNS query loop
 *
 * Solution:
 * Don't swap if one of DNS Server is "0.0.0.0"
 */
      if (curr->ipv4_dns1 && curr->ipv4_dns2 && !cmsUtl_strcmp(dnsUsed, curr->ipv4_dns1)
              && inet_network(curr->ipv4_dns1) != 0 && inet_network(curr->ipv4_dns2) != 0)
#else
      if (curr->ipv4_dns1 && curr->ipv4_dns2 && !cmsUtl_strcmp(dnsUsed, curr->ipv4_dns1))
#endif
      {
