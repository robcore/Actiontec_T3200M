...
#ifdef GPL_CODE
     {
         /*It will create a new list item if the query name is different and transaction id is same*/
         if ((req->message.question[0].type == AAA) || (req->message.question[0].type == A) || (req->message.question[0].type == PTR))
         {
             if ((strlen (req->message.question[0].name) > 0) && (strlen(r->message.question[0].name) > 0) && (strcmp(r->message.question[0].name, req->message.question[0].name) != 0))
             {
                //fprintf(stderr, "transaction id is same but query name is not\n");
                continue;
             }
         }
