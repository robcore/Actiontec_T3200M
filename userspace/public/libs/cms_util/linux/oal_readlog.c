...
int oal_readLogPartial(int ptr, char* buffer)
{
  ...
  // Find starting point for the read
  if (ptr == BCM_SYSLOG_FIRST_READ){
    // If this is the first read, find the first string after the tail
    if (buf->tail == 0 && shbuf_data[0] == '\0' &&
        shbuf_data[1] == '\0' && shbuf_data[2] == '\0') {
        // initial, empty log buffer
        i = buf->tail;
    } else {
      i = buf->tail + strnlen(&(buf->data[buf->tail]), buf->size-buf->tail) + 1;
      if (i >= buf->size) {
         // wrap around to the beginning and try to find first string
         i = strnlen(&(buf->data[0]), buf->size) + 1;
         if (i >= buf->size) {
            cmsLog_error("Could not find starting point! tail=%d", buf->tail);
            i = buf->tail;
         }
      }
    }
  ...
  // i is the index to the start of the read
  if (i == buf->tail) {
...
readnext:
  if ( i != buf->tail) {
    ...
    len = strlen(buffer);
#if defined(GPL_CODE)
    /* if it is already the last log, it should not intend to read the next log, otherwise it will cause the endless read loop */
    if (!((buffer[len] == '\0') &&
                (buffer[len-1] == '\n')) && (i != buf->tail)) {
#else
    if (!((buffer[len] == '\0') &&
      (buffer[len-1] == '\n'))) {
#endif
        // did not get the whole line yet, keep copying (from beginning)
