...
UBOOL8 glbCustomerSpecifyDefConfigFile = FALSE;
#endif
...
CmsRet flashImage(char *imagePtr, UINT32 imageLen, CmsImageFormat format, UINT32 opts, int * noReboot)
{
    ...
    if (format == CMS_IMAGE_FORMAT_FLASH)
    {
        ...
        if ((ret = getFlashInfo(&flags)) == CMSRET_SUCCESS)
        {
           ...
           { // NAND flash device
#ifdef GPL_CODE_CENTURYLINK_UPGRADE_ACTIVE_FW
              if( opts & CMS_IMAGE_WR_OPT_ACTIVE)
                  ret = writeDualImageToNand( imagePtr, imageLen - TOKEN_LEN,
                          CMS_IMAGE_WR_OPT_ACTIVE);
              else
                  ret = writeDualImageToNand( imagePtr, imageLen - TOKEN_LEN,
                          CMS_IMAGE_WR_OPT_INACTIVE);
#else
              ret = writeImageToNand( imagePtr, imageLen - TOKEN_LEN );
#endif
           }
        }
                                    ...
                                    UINT32 opts)
{
   ...
   case CMS_IMAGE_FORMAT_FLASH:
      ...
      update_internet_led_state(AEI_LED_YELLOW_BLINK);
#endif
#if defined(GPL_CODE)
        state = CMS_IMAGE_FLASH_STATUS_UPGRADING;
        cmsImg_storeUpdateState(&state);
#endif
...
#endif
#if defined(GPL_CODE)
        state = CMS_IMAGE_FLASH_STATUS_SUCCESS;
        cmsImg_storeUpdateState(&state);
#endif
#if defined(GPL_CODE_FACTORY_TEST)
         if (noReboot == 0 && forceNoReboot == 0)
#else
         if (noReboot == 0)
#endif
         {
#if defined(GPL_CODE_INTERNET_LED_BEHAVIOR)
            /* set internet LED GREED and sleep 1s, indicate upgrade image success */
            sleep(1);
#endif
...
#ifdef GPL_CODE_DEFAULT_CFG_CUSTOMER
    if(glbCustomerSpecifyDefConfigFile)
    {
      ret = sendConfigMsg(imagePtr, imageLen, msgHandle, CMS_MSG_WRITE_CUSTOMER_DEFAULT_CONFIG_FILE);
...
#if !defined(GPL_CODE)
#if defined(GPL_CODE_USB_LOAD_CONFIGFILE)
         char configinfo[2] = {0};
         cmsPsp_get("Customer_Config", configinfo, sizeof(configinfo));
         if(configinfo[0] == '\0')
#endif
...
#if defined(GPL_CODE_SIGNED_FIRMWARE)
...
#if defined(GPL_CODE)
extern int AEI_devCtl_getBoardID(char *boardid);
CmsRet AEI_verifyImageForBoard(const char * imagePtr, UINT32 imageLen)
{
    WFI_TAG wt = {0}; 
    const char *end_string;
    char boardId[BUFLEN_32] = { 0 };
    const FILE_TAG* pTag = NULL;
    CmsImageFormat ret = CMS_IMAGE_FORMAT_BROADCOM;

    end_string = imagePtr + imageLen - sizeof(wt);
    memcpy(&wt, end_string, sizeof(wt));


    AEI_devCtl_getBoardID(boardId);

    if (((cmsUtl_strcasecmp(boardId, "C1000A") == 0) && (cmsUtl_strcasecmp(boardId, pTag->boardId) != 0)) || ((cmsUtl_strcasecmp(pTag->boardId , "C1000A") == 0) && (cmsUtl_strcasecmp(boardId, "C1000A") != 0)))
    {
        cmsLog_error(" #### boardID:%s, boardId on image:%s\n", boardId, pTag->boardId);
        ret = CMS_IMAGE_FORMAT_WRONG;
    }
...
CmsImageFormat cmsImg_validateImage(const char *imageBuf, UINT32 imageLen, void *msgHandle)
{
...
#endif
   if ((imageLen > sizeof(FILE_TAG)) && (verifyBroadcomFileTag((FILE_TAG *) imageBuf, imageLen) == CMSRET_SUCCESS))
   {
      UINT32 maxLen;
   ...
   {
#if defined(GPL_CODE_CONFIG_JFFS) && defined(GPL_CODE_63168_CHIP)
   ...
   {
      CmsImageFormat retval =  AEI_verifyImageForBoard(imageBuf, imageLen);
      if (retval ==  CMS_IMAGE_FORMAT_WRONG)
      {
         cmsLog_error("#### AEI_verifyImageForBoard return CMS_IMAGE_FORMAT_WRONG \n");
         return retval;
      }
   }
#endif
...
{
#if defined(GPL_CODE)
   //optimize the code only DUT reboot save history log to flash, avoid save flash frequently
   char        *szTmpFile = DEVICE_HISTORY_LOG_FILE;
   char        *szLogFile = DEVICE_HISTORY_FLASH_LOG_FILE;
   char    myCmd[256];
...
#if defined(GPL_CODE)
   /* fix for QA-Bug #228114
    * in some cases, smd failed get the REBOOT msg,
    * lead HTTPD hold cms lock and wait smd reply all the time */
   if(cmsMsg_sendAndGetReplyWithTimeout(msgHandle, &msg, 5*MSECS_IN_SEC) != CMSRET_SUCCESS) //5s
   {
       cmsLog_error("get CMS_MSG_REBOOT_SYSTEM Reply from SMD timeout, try again");
       cmsMsg_sendAndGetReplyWithTimeout(msgHandle, &msg, 5*MSECS_IN_SEC);
   }
#else
   cmsMsg_sendAndGetReply(msgHandle, &msg);
#endif
...
CmsRet cmsImg_storeUpdateState(int *pState)
{
   ...
   if ((ret = cmsPsp_set(CMS_IMAGE_UPDATE_STATE_PSP_KEY, pState, sizeof(int))) != CMSRET_SUCCESS)
   {
      cmsLog_error("set of CMS_IMAGE_TRANSFER_STATS_PSP_KEY failed, ret=%d", ret);
   }
   return (ret);
...
CmsRet cmsImg_getUpdateState(int *pState)
{
   ...
   byteRead = cmsPsp_get(CMS_IMAGE_UPDATE_STATE_PSP_KEY, pState, sizeof(int));
   if (byteRead > 0)
   {
      ret = CMSRET_SUCCESS;
   }
