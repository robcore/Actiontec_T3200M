...
CmsRet parse_eid_line(const char *line, UINT32 len, CmsEntityInfo *eInfo)
{
   ...
   case EID_TOKEN_PORT:
      ...
      if ((eInfo->eid == EID_TR69C) && (eInfo->port != (UINT16)TR69C_CONN_REQ_PORT))
          eInfo->port = (UINT16)TR69C_CONN_REQ_PORT;
#ifdef SUPPORT_HTTPD_SSL
      if (!strcmp(pair.valuebuf, "HTTPD_PORT_SSL"))
      {
         eInfo->port = (UINT16) symbol_to_value("HTTPD_PORT_SSL", strlen("HTTPD_PORT_SSL")+1);
      }
#endif
#ifdef DESKTOP_LINUX
      if (eInfo->port < 1024)
