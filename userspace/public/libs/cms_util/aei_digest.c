...
static void MD5Transform(u_int32_t *buf, u_int32_t *in);
...
MD5Update(struct MD5Context *ctx, u_char const *buf, u_int len)
{
        ...
        if (t) {
                ...
                MD5Transform((uint32_t *)ctx->buf, (u_int32_t *) ctx->in);
                buf += t;
                len -= t;
        }
 ...
 */
void
MD5Transform(u_int32_t *buf, u_int32_t *in)
{
...
/* Get MD5 hash of "wirelessconfig:serialnum" per Bell Alliant to be used
 * for generating default WPA/WPA2 PSK key
 */
...
#if defined(GPL_CODE)
        sprintf((char *)tmp,"Opt1kTV!:%s", (char *)inputStr);
#else
        sprintf((char *)tmp,"wirelessconfig:%s", (char *)inputStr);
#endif
        MD5Context mdContext;

        /* Generate hash */
        MD5Init(&mdContext);
        MD5Update(&mdContext, tmp, strlen((char *)tmp));
...
char *string;
{
  ...
  MD5Init (&context);
  MD5Update (&context, (u_char *)string, len);
  MD5Final (digest, &context);

  printf ("MD5 (\"%s\") = ",string);
  MDPrint (digest);
  printf ("\n");
  MD5ToAscii(digest,ascii);
}
...
1234567890123456789012345678901234567890");
  MD5String("wirelessconfig:CVDA9281800012");
}
