...
CmsRet prctl_setScheduler(SINT32 pid, SINT32 policy, SINT32 priority)
{
   return (oal_setScheduler(pid, policy, priority));
}


CmsRet prctl_setCpuMask(SINT32 pid, UINT32 cpuMask)
{
   return (oal_setCpuMask(pid, cpuMask));
}


CmsRet prctl_setCgroup(SINT32 pid, const char *groupBase, const char *groupName)
{
   return (oal_setCgroup(pid, groupBase, groupName));
}
...
{
   return (oal_getNameByPid(pid, nameBuf, nameBufLen));
}
