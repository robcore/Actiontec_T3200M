   ...
   while (length > 0)
   {
      while (i < 4 && length > 0)
      {
	    ...
	    if (i < 4)
       {
	    ...
	    cmsAst_assert((out - data->output_buffer + 3) <= (SINT32) data->output_buflen);
 ...
 */
static CmsRet pl_base64_decode_flush (PLBase64Decoder *data)
{
    int count;
 ...
 */
static UINT32 PL_Base64MaxDecodedLength (UINT32 size)
{
    return ((size * 3) / 4);
}
...
{
    return ((PLBase64Decoder *) cmsMem_alloc(sizeof(PLBase64Decoder), ALLOC_ZEROIZE));
}
...
CmsRet cmsB64_decode(const char *base64Str, UINT8 **binaryBuf, UINT32 *binaryBufLen)
{
    ...
    {
        cmsLog_error("could not create decoder, cannot decode");
        return CMSRET_INTERNAL_ERROR;
    }
    ...
    data->output_buflen = PL_Base64MaxDecodedLength(base64StrLen);
    data->output_buffer = cmsMem_alloc(data->output_buflen, ALLOC_ZEROIZE);
    
    if (data->output_buffer == NULL)
    {
       cmsLog_error("could not allocate output buffer of %d bytes", data->output_buflen);
       PL_DestroyBase64Decoder(data);
       return CMSRET_RESOURCE_EXCEEDED;
    }

    ret = pl_base64_decode_buffer(data, (const unsigned char *) base64Str, base64StrLen);
    ...
    if (ret == CMSRET_SUCCESS)
    {
       ...
       *binaryBuf = data->output_buffer;
       data->output_buffer = NULL; /* Must clear this or Destroy will free it. */ 
    }
