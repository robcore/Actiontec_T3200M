...
#if defined(GPL_CODE_CONFIG_JFFS) && defined(CONFIG_BCM963138)
...
#if defined(GPL_CODE) && defined(GPL_CODE_TWO_IN_ONE_FIRMWARE)
          fprintf(stderr, "--size=%d signiture_2=[%s] imageVersion=[%s] mageVersion_2=[%s]\n", size, pTag->signiture_2, pTag->imageVersion, pTag->imageVersion_2);
          memset (boardId, 0, sizeof(boardId));
          AEI_devCtl_getBoardID(boardId);
          if (!strcmp(boardId,"C2000A"))
          {
              if( size > sizeof(pTag->imageVersion) )
                 len = sizeof(pTag->imageVersion);
              else
                 len = size;
              memcpy(string, pTag->imageVersion, len);
          }
          else if (!strcmp(boardId,"C1900A"))
          {
              if( size > sizeof(pTag->imageVersion_2) )
                 len = sizeof(pTag->imageVersion_2);
              else
                 len = size;
              memcpy(string, pTag->imageVersion_2, len);
          }
          else
...
unsigned int AEI_getSequenceNumber(int imageNumber)
{
    ...
    if ( seq == 0 )
       seq = getSequenceNumber(imageNumber);
