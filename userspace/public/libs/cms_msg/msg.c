...
extern int set_msg_socket_snd_timeout(int fd, UBOOL8 *blkState, UBOOL8 noblock, const CmsMsgHeader *hdr);
#endif
...
CmsRet cmsMsg_send(void *msgHandle, const CmsMsgHeader *buf)
{
   ...
   if( (ret = oalMsg_send(handle->commFd, buf)) != CMSRET_SUCCESS){
       if(errno==EAGAIN)
           set_msg_socket_snd_timeout(handle->commFd, &handle->sndNoBlock, TRUE, buf);
   }else{
       set_msg_socket_snd_timeout(handle->commFd, &handle->sndNoBlock, FALSE, buf);
   }
   return ret;
#else
...
#if defined(GPL_CODE_CMS_MSG_NOBLOCK)
   CmsRet ret = CMSRET_SUCCESS;
   if( (ret = oalMsg_send(handle->commFd, &replyMsg)) != CMSRET_SUCCESS){
       if(errno==EAGAIN)
           set_msg_socket_snd_timeout(handle->commFd, &handle->sndNoBlock, TRUE, &replyMsg);
   }else{
       set_msg_socket_snd_timeout(handle->commFd, &handle->sndNoBlock, FALSE, &replyMsg);
   }
   return ret;
#else
...
#if defined(GPL_CODE_CMS_MSG_NOBLOCK)
   if(oalMsg_send(handle->commFd, &noopMsg) != CMSRET_SUCCESS){
       if(errno==EAGAIN)
           set_msg_socket_snd_timeout(handle->commFd, &handle->sndNoBlock, TRUE, &noopMsg);
   }else{
       set_msg_socket_snd_timeout(handle->commFd, &handle->sndNoBlock, FALSE, &noopMsg);
   }
#else
