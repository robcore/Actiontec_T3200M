...
#if defined(GPL_CODE)
    if (disc_sock > 0) {
        /*WindStream Request WAN keep up, SET PPPoE client highest priority
         *PPP(LCP, IPCP etc) Traffic set by ppp_write() in kernel
         */
        unsigned int mark = 0x7;
        setsockopt(disc_sock, SOL_SOCKET, SO_MARK, (char *)&mark, sizeof(mark));
    }   
#endif 
