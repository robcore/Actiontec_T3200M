...
#if defined(GPL_CODE)
/* support T3200 */
#define IS_SUPPORT_SENSOR(board_id) (!strncmp(board_id, "T3200", 5) || !strncmp(board_id, "C3000", 5) || !strncmp(board_id, "F3500", 5))
static struct spi_board_info sensor_spi_slave_info[] = {
	{
        	.modalias        = "lis3lv02d_spi",
		.controller_data = (void *)(1 << 29), //SPI_CONTROLLER_STATE_GATE_CLK_SSOFF
		.chip_select     = 1,
		.max_speed_hz    = 5000000,
		.bus_num         = 1, //HS_SPI_BUS_NUM,
		.mode            = 3, //SPI_MODE_DEFAULT,
	},
};

static void __init actiontec_setup(void)
{
    char p[NVRAM_BOARD_ID_STRING_LEN];

    kerSysNvRamGetBoardId(p);
    if (!IS_SUPPORT_SENSOR(p))
	return;

    printk("Initializing 3D sensor Hardware\n");
    spi_register_board_info(sensor_spi_slave_info,
	    ARRAY_SIZE(sensor_spi_slave_info));
    printk("Added LIS3x position sensor\n");
}
#endif
