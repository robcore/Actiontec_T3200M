	...
	const struct pt_regs *regs)
{
	...
	while ((unsigned long) sp & (PAGE_SIZE - 1)) {
		if (i && ((i % (64 / field)) == 0))
			printk("\n       ");
#ifndef GPL_CODE   /* Actiontec: Always print the entire stack in case of kernel panic */
		if (i > 39) {
			printk(" ...");
			break;
		}
...
void dump_stack(void)
{
	...
	prepare_frametrace(&regs);
#ifdef GPL_CODE   /* Actiontec: Always print the entire stack in case of kernel panic */
	show_registers(&regs);
#else
