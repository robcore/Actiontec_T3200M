...
//#define AEI_TX_HANGUP_WORKAROUND 1
...
#if defined(GPL_CODE_TX_HANGUP_WORKAROUND)
	struct rqinfo* rqinfolist;
	uint32_t infowindex;
#endif

#if defined(GPL_CODE)
	spinlock_t aei_tx_lock;
	struct sk_buff_head aei_teardown_queue;
#endif
#if defined(GPL_CODE_TOOLBOX)
        uint16_t usMirrorInFlags;
        uint16_t usMirrorOutFlags;
#endif
};
