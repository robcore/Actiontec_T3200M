...
#if defined(GPL_CODE)
static int _aei_mtd_lock = 1;
void aei_mtd_lock(void)
{
    _aei_mtd_lock = 1;
}
EXPORT_SYMBOL_GPL(aei_mtd_lock);
void aei_mtd_unlock(void)
{
    _aei_mtd_lock = 0;
}
EXPORT_SYMBOL_GPL(aei_mtd_unlock);
#endif
...
#if defined(GPL_CODE)
	/*Don't allow to mount bootfs with R/W mode*/
	if (_aei_mtd_lock && strstr(mtd->name, "bootfs") && !(flags & MS_RDONLY))
