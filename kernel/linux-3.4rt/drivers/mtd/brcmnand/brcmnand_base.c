...
static int brcmnand_erase(struct mtd_info *mtd, struct erase_info *instr)
{
...
#ifdef GPL_CODE_NAND_CNT_128K
#ifdef GPL_CODE_FLASH_CNT_DEBUG
	printk("*** [MTD] Erase flash at %08lx, %d Byte ***\n",(unsigned long) instr->addr, (int)instr->len);
#endif

	erase_addr = instr->addr;
	
    memset(&ops, 0x00, sizeof(ops));
    memset(oobbuf, 0x00,64);
	...
	//printk("read oob ....@ %08llx \n",erase_addr);
	brcmnand_get_device(mtd, FL_READING);
    sts = brcmnand_do_read_oob(mtd, erase_addr, &ops);
    brcmnand_release_device(mtd);
    
	//printk("read oob sts =%d\n",sts);
	//print_oobbuf(oobbuf,64);

	if(sts == 0)
	{
    	memcpy((char *)erase_cnt,(char *)oobbuf+16,4);
    	p_count = (unsigned int *)erase_cnt;
    	//printk("%08llx erase times = %d \n",erase_addr,(*p_count));
    	(*p_count)++;
    }
    else
    	printk("read oob from addr:%08llx failed!!!\n",erase_addr);
#endif 
	...
	if(sts == 0)
	{
    	...
    	ops.ooboffs = 16;
    	ops.datbuf  = NULL;
    	ops.oobbuf  = erase_cnt;
    	ops.len     = 0;
    	ops.mode    = MTD_OPS_RAW;

		brcmnand_get_device(mtd, FL_WRITING);
    	sts = brcmnand_do_write_oob(mtd, erase_addr, &ops);
    	brcmnand_release_device(mtd);
    	
    	if(sts != 0)
    		printk("write oob to addr:%08llx failed!!!\n",erase_addr);
	}
#endif
