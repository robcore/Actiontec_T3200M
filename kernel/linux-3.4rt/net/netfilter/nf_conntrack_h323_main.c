...
#endif
#if defined(CONFIG_NF_CONNTRACK_ALG_QOS)
...
#if defined(CONFIG_NF_CONNTRACK_ALG_QOS)
	if (rtp_exp->tuple.dst.u.udp.port > 0)
	{
		struct nf_conntrack_tuple *tuple;

		tuple = &(rtp_exp->tuple);

		aei_alg_qos_handle_conn(ALG_QOS_ACTION_ADD_CONN, ALG_QOS_PROTO_H323, tuple);
	}
	if (rtcp_exp->tuple.dst.u.udp.port > 0)
	{
		struct nf_conntrack_tuple *tuple;

		tuple = &(rtcp_exp->tuple);

		aei_alg_qos_handle_conn(ALG_QOS_ACTION_ADD_CONN, ALG_QOS_PROTO_H323, tuple);
	}

	ct->alg_qos_proto = ALG_QOS_PROTO_H323;
#endif
