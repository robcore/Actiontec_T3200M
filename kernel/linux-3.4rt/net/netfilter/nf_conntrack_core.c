...
void nf_ct_delete_from_lists(struct nf_conn *ct)
{
	...
	if (ct && (ct->alg_qos_proto > 0))
	{
		struct nf_conntrack_tuple *tuple;

		tuple = &(ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple);
		aei_alg_qos_handle_conn(ALG_QOS_ACTION_DEL_CONN, ct->alg_qos_proto, tuple); 
	}
#endif
...
#endif
#if defined(CONFIG_NF_CONNTRACK_ALG_QOS)
		ct->alg_qos_proto = exp->master->alg_qos_proto;
#endif
