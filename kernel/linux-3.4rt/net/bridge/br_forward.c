...
#endif
{
	...
	if( (skb->dev->priv_flags & p->dev->priv_flags) & IFF_WANDEV
                            ...
                            && !(p->dev->priv_flags & IFF_AEI_WAN_TO_LAN)
#endif
        )
	{
		return 0;
...
#if defined(GPL_CODE_BRIDGE_STB_QOS)
void aei_get_ip_udp_hdrs( const struct sk_buff *pskb, struct iphdr **ppiphdr, struct udphdr **ppudphdr)
{
	...
	*ppudphdr = NULL;
	if ( pip != NULL )
	{
		...
		{
			pudp = (struct udphdr *)((char *)pip + (pip->ihl << 2));
		}
		*ppiphdr = pip;
		*ppudphdr = pudp;
	}
...
static void __br_forward(const struct net_bridge_port *to, struct sk_buff *skb)
{
	...
	if (skb_warn_if_lro(skb)) {
		...
		aei_get_ip_udp_hdrs(skb, &piphdr, &pudphdr);
		if (piphdr && pudphdr && (ntohs(pudphdr->dest) == DHCP_SVR_PORT))
		{
			//printk("AEI found dhcp packet\n");
//			aei_handle_dhcp_packet(skb, pudphdr);
		}
        }
#endif
	indev = skb->dev;
