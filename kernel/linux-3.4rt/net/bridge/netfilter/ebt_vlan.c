...
#if defined(GPL_CODE_EBT_VLAN_EXT)
	/* Checking Encapsulated Proto (Length/Type) field */
    if (GET_BITMASK(EBT_VLAN_PACKET_CONTENT))
    {
        if ( strcasecmp(info->pktcontent, "DHCP") == 0 )
        {
            /*use this value to match dhcp packets*/
            ...
            if ( !(vlan_tx_tag_present(skb)) )
            {
                /*in new kernel, skb->data doesn't fill with vlan data*/
                offset +=  VLAN_HLEN;
            }
            ...
            pptr = skb_header_pointer(skb, offset+(ih->ihl*4), sizeof(_ports), &_ports);
            if (pptr == NULL) 
                return false;

            if((ntohs(pptr->src) != 68)&&(ntohs(pptr->dst) != 67))
                return false;
        }
        else 
		    EXIT_ON_MISMATCH(encap, EBT_VLAN_ENCAP);
