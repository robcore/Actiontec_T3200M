...
ebt_ip_mt(const struct sk_buff *skb, struct xt_action_param *par)
{
...
#if defined(GPL_CODE)
	if (info->bitmask & EBT_IP_LEN) {
		u16 tot_len = ntohs(ih->tot_len);
		if (FWINV(tot_len < info->iplen[0] ||
			  tot_len > info->iplen[1],
			  EBT_IP_LEN))
		return false;
	}
#endif
	...
	if (info->bitmask & EBT_IP_DSCP &&
	   FWINV(info->dscp != (ih->tos & 0xFC), EBT_IP_DSCP))
		return false;
        if (info->bitmask & EBT_IP_TCPFLAG) {
#define FWINVTCP(bool, invflg) ((bool) ^ !!(info->invflags & (invflg)))
           if (ntohs(ih->frag_off) & IP_OFFSET)
                return false;
           th = skb_header_pointer(skb, ih->ihl*4, sizeof(_tcph), &_tcph);
           if (th == NULL) 
                return false;
           if (!FWINVTCP((((unsigned char *)th)[13] & info->flg_mask)
                         == info->flg_cmp,
                         EBT_IP_TCPFLAG))
                return false;
        }
...
static int ebt_ip_mt_check(const struct xt_mtchk_param *par)
{
	...
	if (info->bitmask & EBT_IP_SPORT && info->sport[0] > info->sport[1])
		return -EINVAL;
        if (info->bitmask & EBT_IP_TCPFLAG && info->protocol != IPPROTO_TCP)
                return -EINVAL;
#if defined(GPL_CODE)
	if (info->bitmask & EBT_IP_LEN && info->iplen[0] > info->iplen[1])
		return -EINVAL;
#endif
	return 0;
}
