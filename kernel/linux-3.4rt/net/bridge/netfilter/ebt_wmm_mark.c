...
#if defined(CONFIG_BCM_KF_IP) && defined(CONFIG_IPV6)
#ifdef GPL_CODE
        //I think, we should also can configure wmm ARP priority.
        if ((e->ethproto != __constant_htons(ETH_P_IPV6) && e->ethproto != __constant_htons(ETH_P_IP) && e->ethproto != __constant_htons(ETH_P_8021Q) && e->ethproto != __constant_htons(ETH_P_ARP)) ||
#else
...
#ifdef GPL_CODE
        //I think, we should also can configure wmm ARP priority
        if ((e->ethproto != __constant_htons(ETH_P_IP) && e->ethproto != __constant_htons(ETH_P_8021Q) && e->ethproto != __constant_htons(ETH_P_ARP)) ||
#else  
