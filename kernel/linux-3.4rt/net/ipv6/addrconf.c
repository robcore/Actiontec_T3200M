...
#define DAD_RESEND_TIMES	(200)
#define DAD_RESEND_DURA		(HZ/2)

static void broadcast_resend_dad(unsigned long);
static int broadcast_resend_dad_times=DAD_RESEND_TIMES;
static DEFINE_TIMER(resend_dad_timer, broadcast_resend_dad, 0, 0);
...
static int addconf_broadcast_dad_failed(struct inet6_ifaddr *ifp) {
	...
	if (ret < 0) {
		if (broadcast_resend_dad_times>0) {
			//printk(KERN_ERR "Need redo broadcast start a timer\n");

			resend_dad_timer.expires = jiffies + DAD_RESEND_DURA;
			resend_dad_timer.data = (unsigned long)ifp;

			mod_timer(&resend_dad_timer,resend_dad_timer.expires);
			broadcast_resend_dad_times--;
		}
	}
	else if( timer_pending(&resend_dad_timer)) {
		del_timer(&resend_dad_timer);
		printk(KERN_ERR "Sent succ,need delete the timer in pending");
	}
	printk(KERN_ERR "====>Broadcast DAD failed msg ,ret is %d\n",ret);
...
{
	struct inet6_ifaddr *flag;
	if (!param) {
		printk(KERN_NOTICE "Param in broadcast time func is null,should be a bug\n");
		return;
	}
	flag=(struct inet6_ifaddr *)param;
	addconf_broadcast_dad_failed(flag);
	return;
}
...
#ifdef ACTION_TEC_IPV6_CODE_FOR_IOT
if(ifp->idev->dev->priv_flags & IFF_WANDEV)
...
void addrconf_prefix_rcv(struct net_device *dev, u8 *opt, int len, bool sllao)
{
	...
	if (!pinfo->onlink) {
		printk(KERN_DEBUG "According to CPE.2.3 Part C,wouldn't accept this Prefix\n");
		return;
	}
#endif
	/*
