			...
			struct netdev_queue *txq)
{
	...
	if (likely(!skb->next)) {
		...
		if (netif_needs_gso(skb, features)) {
            ...
            /* the skb->protocol should be the pre protocl(ip or ipv6)
             * but the skb->protocol is set to ETH_P_802_3 in origin 
             * code . this bug will cause the csum check incorrect . 
             * refs to solution of C2000 : it will not csum check when 
             * process the mirror packet . 
             */
			if (skb->ip_summed == CHECKSUM_PARTIAL && skb->protocol != ETH_P_802_3) {
#else
...
static int dev_ifsioc(struct net *net, struct ifreq *ifr, unsigned int cmd)
{
	...
	case SIOCSHWTSTAMP:
		...
		case SIOCGIFTRANSSTART:
			...
			return 0;
#if defined(GPL_CODE_IOCTL_IFSTATS) || defined(GPL_CODE_STATS_DIAG)
