...
/* 
 * First 4 bytes of GRE interface MAC address are derived from the
 * Tunnel Local IPv4 address. In such a case, depending on the 
 * value of the least significant byte of IPv4 address, the 
 * 'Unicast/Multicast' bit and the 'Global/Local assignment' bits
 * in the MAC address might be set to 1.
 * We want the GRE interface MAC to be Locally administered Unicast MAC.
 */
static void ipgre_sanitize_ether_addr(u8 *addr)
{
...
#ifdef GPL_CODE_COMMUNITY_WIFI
                                ipgre_sanitize_ether_addr(dev->dev_addr);
#endif
